...
| 2026-08-01 | [CTA Inventory & Redesign](2026-08-01_CTA_INVENTORY_AND_REDESIGN.md) | `5efe3c4f` | 9 button styles → 6. **`cta stay` is a phantom** (3 sites, 0 rules) — 2nd occurrence after `cta big`, so gate it. `.confrow` **and its parent** are hero-only. Redesign: kill the 180° gradient (no leader ships one), pill 750 is bolder than the primary 700, tap floor is on the wrong button |
