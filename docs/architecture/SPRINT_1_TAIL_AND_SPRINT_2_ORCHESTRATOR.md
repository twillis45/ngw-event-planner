...
| Piece | File | State |
|---|---|---|
| Secure server-side AI proxy | `backend/app/routers/ai.py` | **Exists.** Routes: `/status`, `/feature`, `/extract-document`, `/parse-vendor-reply`, `/orchestrate`. Key server-side, planner-authenticated, rate-limited. `/complete` was **removed** — it accepted a caller-supplied system prompt and had no reachable consumer; see `docs/security/AI_PROXY_AND_DOCUMENT_FETCH_SECURITY.md`. |
