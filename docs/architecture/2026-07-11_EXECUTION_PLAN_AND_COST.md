...
**One-line summary (corrected 2026-07-17):** **Sprint 1 is done bar the legacy DELETION** — the freeze shipped and the undo was already universal when this ledger called it partial; what's left there is a mechanical pre-delete diff, not a decision. Sprints 5–6 are untouched.

> **Known stale, NOT yet corrected here (2026-07-17):** the Sprint 2 row still reads *"the true server-side tool-calling orchestrator does not exist yet."* That was true when written and is not now — it exists, and its acceptance passed live against a real key. Only the Sprint 1 rows above have been re-verified against the code; treat Sprint 2–4 below as unaudited until someone does the same for them.
