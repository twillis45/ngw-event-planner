...
// Explicit type→family registry. EACH event type has its OWN model — there is NO
// wedding default. An unrecognized type returns null (no preview) rather than
// fabricating wedding milestones. Order matters: specific patterns before generic.
const FAMILY_BY_TYPE = [
  // on-trend + specific ceremonies (most specific first)
  [/bachelor|bachelorette|hen\s*(do|party)|stag/, 'bachelorette'],
  [/proposal|will\s*you\s*marry/, 'proposal'],
  [/gender\s*reveal|sex\s*reveal/, 'gender_reveal'],
  [/elope|micro.?wedding|minimony/, 'elopement'],
  [/wellness|yoga|sound\s*bath|breathwork|meditation\s*retreat/, 'wellness_retreat'],
  [/quince/, 'quinceanera'],
  [/sweet\s*16|sweet\s*sixteen/, 'sweet16'],
  [/baby\s*shower|baby\s*sprinkle|sip\s*(and|&)\s*see/, 'baby_shower'],
  [/bridal\s*shower|wedding\s*shower/, 'bridal_shower'],
  [/engagement/, 'engagement_party'],
  [/vow\s*renewal/, 'vow_renewal'],
  [/anniversary/, 'anniversary'],
  [/graduation|grad\s*party/, 'graduation'],
  [/retirement/, 'retirement_party'],
  [/reunion/, 'reunion'],
  // corporate family
  [/board\s*meeting|board|trustee|governance/, 'board'],
  [/product\s*launch|launch\s*party/, 'product_launch'],
  [/town\s*hall|all.?hands/, 'town_hall'],
  [/training|workshop|seminar|class/, 'training_workshop'],
  [/award|recognition\s*ceremony/, 'award_ceremony'],
  [/client\s*dinner|client\s*entertain|business\s*dinner/, 'client_dinner'],
  [/team\s*retreat|offsite|team\s*build/, 'team_retreat'],
  [/holiday\s*party|company\s*party|staff\s*party/, 'holiday_party'],
  [/conference|summit|kickoff|\bsko\b/, 'corporate'],
  // social & fundraising
  [/gala|fundrais|benefit\b/, 'gala'],
  [/networking|mixer/, 'networking_event'],
  // individually hosted casual
  [/get.?together|house\s*warming|housewarming|dinner\s*party|friendsgiving|game\s*night|potluck|cookout|bbq|backyard|brunch|happy\s*hour|watch\s*party/, 'home_gathering'],
  // celebrations (generic, after specifics)
  [/wedding/, 'wedding'],
  [/birthday|\bb.?day\b|milestone|sweet|party\b/, 'birthday'],
  // generic corporate fallback (only for clearly-corporate language)
  [/corporate|meeting|conference|company|business/, 'corporate'],
];
function familyFor(event) {
  const t = ((event && (event.type || event.kind || event.category)) || '').toLowerCase();
  for (const [re, fam] of FAMILY_BY_TYPE) if (re.test(t)) return fam;
  return null; // honest: no model yet → no preview. NEVER silently a wedding.
}
