...
const conference = {
  ...
  decisions: [
    { id: 'format', label: 'In-person, hybrid, or virtual?', options: ['In-person only', 'Hybrid (in-person + streamed)', 'Virtual only'], default: 'In-person only', defaultWhy: 'In-person only is the default because hybrid roughly doubles the AV and production line — capture, streaming, a virtual platform, a remote-moderator workflow — and reshapes registration and sponsor packages with it. Change it if reaching the people who cannot travel is part of what this event is for.', when: 'T-180d', blocks: ['vendors', 'av_production', 'registration'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'This is the single biggest scope lever — hybrid roughly doubles the production line and reshapes registration and sponsor packages, so it steers the whole budget.', tier: 'reasoned' }, why: 'The single biggest scope lever. Hybrid roughly doubles the AV/production line (capture, streaming, a virtual platform, a remote-moderator workflow) and changes registration + sponsor packages.' },
    { id: 'tracks', label: 'Single-track or multi-track agenda?', options: ['Single track (one room)', 'Two tracks', 'Multi-track (3+ breakouts)'], default: 'Multi-track (3+ breakouts)', defaultWhy: 'Multi-track is the default because it is the show this playbook plans — a general session, breakout tracks and an expo floor. Change it to a single track if your program does not need the choice: every track you drop is a room you do not contract, an AV kit you do not rent, and a tech you do not staff.', when: 'T-150d', blocks: ['agenda', 'venue_rooms', 'av_production'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Track count decides how many breakout rooms you contract and how many AV kits and techs you staff — it multiplies the two most expensive lines.', tier: 'reasoned' }, why: 'Drives how many breakout rooms you contract, how many AV kits + techs you need, and the room-set / turnover plan. Multi-track multiplies AV and signage.' },
    { id: 'ticketing', label: 'Ticketed (paid) or free registration?', options: ['Paid / ticketed', 'Free (sponsor-funded)', 'Invite-only / comped'], default: 'Paid / ticketed', when: 'T-150d', blocks: ['registration', 'budget_model'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'The revenue model sets the registration platform, the refund policy, and the no-show buffer — a business call only the organizer can make.', tier: 'reasoned' }, why: 'Sets the registration platform, payment/ticketing flow, refund policy, and the no-show buffer you build into the F&B guarantee.' },
    { id: 'sponsor_model', ask: 'Tiered sponsorships, an expo floor, both, or neither?', label: 'Sponsor / exhibitor model', options: ['Tiered sponsorships (Gold/Silver/Bronze)', 'Expo booths only', 'Sponsorships + expo floor', 'No sponsors'], default: 'Sponsorships + expo floor', when: 'T-150d', blocks: ['expo_floor', 'budget_model', 'signage'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'Sponsorship is usually the largest revenue line, so the tier structure and expo plan shape both the budget and the floor — the org owns that strategy.', tier: 'reasoned' }, why: 'Often the largest revenue line for a conference. Sets the prospectus, the expo floor plan, booth inventory, and what each tier gets (logo placement, stage time, badge scans).' },
    { id: 'room_block', ask: 'One hotel block with shuttles, several, or none at all?', label: 'Hotel room block + shuttle plan', options: ['Headquarter hotel block + shuttles', 'Multiple hotel blocks', 'Venue-attached hotel (no shuttle)', 'No block (attendees self-book)'], default: 'Headquarter hotel block + shuttles', when: 'T-150d', blocks: ['transport', 'attrition_clause'], dependsOn: ['format'], weight: 'med', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'A room block carries an attrition clause that bills unsold rooms back to you, so it is a real financial commitment, not just a convenience.', tier: 'reasoned' }, why: 'A room block carries an attrition clause — unsold rooms can bill back to you. Drives shuttle routing/timing and the comp-room math in your contract.' },
  ],
  ...
  purchases: [
    { id: 'p_badges', item: 'Attendee badges + lanyards', category: 'logistics', qtyPerGuest: 1, unit: 'badge', where: ['Registration vendor', 'Print shop', 'Amazon'], unitCostRange: [1.5, 5], essential: true, buyAt: 'T-3d', note: 'Order ~10% over final count for walk-ins, reprints, staff, and VIP/speaker ribbons.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['badges-print-2026', 'lanyards-retail-2026'], lastVerified: '2026-08-18', claim: 'Per-badge cost including the lanyard. BADGES: printed cardboard from $2.19 each; plastic ID name badges $4.37 each at 25-74 quantity; bulk packages from $10.25 per 25, about $0.41 a badge. LANYARDS: $0.21 each blank in bulk, about $0.99 added to a badge order. A conference-scale order of cardboard badges plus bulk lanyards lands mid-band; a small order of plastic badges with printed lanyards reaches the ceiling.', sufficientWhen: 'A per-badge quote at the actual attendee count, plus a lanyard unit price, confirm where in the band an order lands.' }, },
    { id: 'p_signage', item: 'Directional + wayfinding signage (foam board, banners, room cards)', category: 'decor', qtyFlat: 1, unit: 'sign package', where: ['Sign/print shop'], unitCostRange: [800, 4000], essential: true, buyAt: 'T-3d', note: 'Scales with venue size + track count: arrivals, registration, each breakout room, expo, restrooms.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['signage-print-2026', 'banner-retail-2026'], lastVerified: '2026-08-18', claim: 'A package of printed wayfinding signage, not a single sign. Foam board runs $20-50 per square foot - an 18x24in sign $15-30, a 4x8ft display $150-400 - and vinyl banners $50-200, with 8x10ft large-format at $150-400. A conference needs directional signs at multiple decision points plus room identifiers and a main banner, so the package multiplies those unit figures; the band\'s ceiling is a large venue with many rooms. Foam board is INDOOR only (UV yellows it in 3-6 months), which is why outdoor wayfinding prices at the vinyl or corrugated tier instead.', sufficientWhen: 'A quote for the actual sign count and sizes at one printer confirms where in the band a given venue lands.' }, },
    { id: 'p_programs', item: 'Printed programs / pocket agenda', category: 'logistics', qtyPerGuest: 1, unit: 'program', where: ['Print shop'], unitCostRange: [1, 4], essential: false, buyAt: 'T-3d', note: 'Often replaced by the attendee app; print a short pocket card or signage if going app-only.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['print-booklets-2026', 'print-binding-dividers-2026'], lastVerified: '2026-08-18', claim: 'Printed programs per program, from a published price list. A half-page 5.5x8.5in 8-page booklet is $227 for 50 ($4.54 each), $252 for 100 ($2.52), $356 for 200 ($1.78) and $451 for 250 ($1.80); a full-page 8.5x11in runs $4.82 at 50 down to $2.13 at 250. QUANTITY IS THE WHOLE BAND - a 50-copy run costs more than twice per piece what a 200-copy run does, so a small conference sits at the ceiling and a large one at the floor.', sufficientWhen: 'A booklet quote at the actual attendee count and page count confirms where in the band it lands.' }, },
    { id: 'p_swag', item: 'Swag bags + contents (notebook, pen, water bottle)', category: 'logistics', qtyPerGuest: 1, unit: 'bag', where: ['Promo vendor', 'Sponsor-supplied'], unitCostRange: [5, 25], essential: false, buyAt: 'T-3d', note: 'Frequently sponsor-funded or sponsor-branded; confirm who pays + who fills the bags.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['swagbags-blank-2026', 'promo-notebooks-2026', 'waterbottles-blank-2026', 'meetingsupplies-quill-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced components. BAG: a blank non-woven drawstring is $1.10-2.18, cotton canvas $2.18-3.78. NOTEBOOK: custom-printed $3.21-5.86 a unit depending on quantity, PLUS a $40 per-colour setup charge spread across the run; a blank retail notebook is $1.61-4.49. PEN: $0.10-0.19 in bulk. WATER BOTTLE: blank plastic $2.84-3.61, custom-printed $0.79-3.45 at high minimums. Summed, a modest bag lands near this band\'s floor and a printed-notebook-plus-bottle bag near its ceiling.', sufficientWhen: 'A quote for the actual attendee count covering bag, notebook, pen and bottle, with setup stated separately, confirms the band.' }, },
    { id: 'p_reg_supplies', item: 'Registration desk kit (printers, scanners, badge stock, signage, supplies)', category: 'logistics', qtyFlat: 1, unit: 'kit', where: ['Registration vendor', 'Office supply'], unitCostRange: [300, 1500], essential: true, buyAt: 'T-1d', note: 'On-site badge printing, scanners, power strips, extension cords, gaff tape, sharpies, scissors.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['registration-purchase-2026', 'registration-rental-2026'], lastVerified: '2026-08-18', claim: 'A registration desk kit, and the band spans BUY versus RENT. PURCHASE: a badge printer is $711.43-967.65 and a scanner with stand $208.40-209.00, so buying outright starts near $920. RENTAL: printers are $50-200 a day, a printer-plus-compute-plus-scanner bundle about $410 an event, and packages start at $600, with delivery and collection $250 each per kiosk. CONSUMABLES are separate: badge stock $199 per 500, ink about $120. EVIDENCE CAVEAT: the printer figures come from two resellers quoting identical prices - one price level confirmed twice.', sufficientWhen: 'A rental quote for the actual event length against a purchase quote for the same equipment confirms which side of the band applies.' }, },
    { id: 'p_water_stage', item: 'Speaker/stage hospitality (bottled water, throat lozenges, stage snacks)', category: 'beverage', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Venue F&B'], unitCostRange: [40, 150], essential: false, buyAt: 'T-1d', note: 'Green-room + stage water for speakers; most attendee F&B is on the BEO, not bought.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['firstaid-consumables-2026', 'bottledwater-case-2026', 'chips-retail-2026'], lastVerified: '2026-08-18', claim: 'Stage and speaker hospitality per kit. THROAT LOZENGES: $5.19-8.99 for a 50-count box and $8.29-14.99 for 125. BOTTLED WATER: $0.13-0.25 a bottle in a 24-pack. STAGE SNACKS price with the retail snack tier. Provisioning a green room for a multi-day event across several speakers is what carries this band above the individual figures - it is a quantity, not a premium.', sufficientWhen: 'A green-room provisioning list at the actual speaker count and day count, summed, confirms the band.' }, },
    { id: 'p_signage_blanks', item: 'Blank tent cards, name plates, easels, table numbers', category: 'decor', qtyFlat: 1, unit: 'set', where: ['Office supply', 'Party store'], unitCostRange: [50, 250], essential: false, buyAt: 'T-1d', note: 'Head-table / panel name tents and breakout room reserved-seat cards.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['tablenumbers-holders-2026', 'print-booklets-2026', 'signage-print-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced desk and table items. TABLE NUMBERS: plastic sets $1.99-5.49 for 1-25, $7.99-14.35 for 1-100 (about $0.08-0.22 a number); stainless tents an order of magnitude higher at $42.99-89.45 a set. BADGE HOLDERS $0.295-0.75 each at 50 a pack. EASELS about $7.50 each in a 12-pack. BLANK TENT CARDS price with the print tier. A conference-scale set of all four brackets this band; choosing stainless tents pushes it to the ceiling.', sufficientWhen: 'Prices for the actual counts of numbers, holders and easels, summed, confirm the band.' }, },
  ],
