...
const elopement = {
  ...
  decisions: [
    { id: 'location', label: 'Where will you elope?', options: ['National / state park', 'Beach / coast', 'Mountain / wilderness', 'Courthouse', 'Destination (out of state / abroad)', 'Private property / Airbnb'], default: 'National / state park', when: 'T-90d', blocks: ['permit', 'license', 'travel', 'photographer'], weight: 'high', reversibility: 'costly', emotionalWeight: 'high', difmCapable: 'needs-host', priorityBasis: { rationale: 'The place you choose is the heart of eloping, and it sets which license rules, permits, and travel apply — the most far-reaching call on the board.', tier: 'reasoned' }, why: 'The single most-blocking decision. Location sets which marriage-license rules apply, whether a permit is required, and how far everyone has to travel.' },
    { id: 'license_jurisdiction', label: 'Which county/state issues your marriage license?', options: ['County of the ceremony location', 'Home county (if same state)', 'Self-solemnize state (e.g. Colorado)', 'Destination/abroad — research separately'], default: 'County of the ceremony location', when: 'T-75d', blocks: ['license'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'Governs whether the marriage is even legal — waiting periods and witness rules vary by state, so the couple must confirm the issuing jurisdiction, not the app.', tier: 'reasoned' }, why: 'License is governed by where you MARRY, not where you live. Waiting periods (0–6 days), validity windows (often ~90 days), and witness counts (0, 1, or 2) all vary by state. Get this wrong and the marriage is not legal.' },
    { id: 'witnesses', ask: 'Is anyone else there, or just the two of you?', label: 'Who (if anyone) comes?', options: ['Just the two of us', 'Couple + officiant only', '1–2 witnesses (family/friends)', 'Small group (up to ~10)'], default: 'Just the two of us', when: 'T-60d', blocks: ['license', 'travel', 'meal'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'high', difmCapable: 'needs-host', priorityBasis: { rationale: 'Who you let into the moment is deeply personal, and some states require a witness to sign the license — a call only the couple can make.', tier: 'reasoned' }, why: 'Some states require 1–2 witnesses to sign the license. If you are eloping truly alone, confirm your state allows it (or that the officiant/a stranger can witness) BEFORE the day.' },
    { id: 'officiant', ask: 'An officiant, a friend, a judge — or the two of you?', label: 'Who marries you?', options: ['Hired officiant', 'Friend ordained online', 'Courthouse judge/clerk', 'Self-solemnize (where legal)'], default: 'Hired officiant', when: 'T-60d', blocks: ['license'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'Someone legally empowered has to solemnize and sign, and not every state accepts an online-ordained friend or self-solemnizing — the couple must verify it before the day.', tier: 'reasoned' }, why: 'Someone legally empowered must solemnize and sign. Online ordination is fine in most states but NOT all — verify the location accepts it.' },
    { id: 'tell_family', ask: 'Does family hear before, after — or come along?', label: 'Whether / how to tell family', options: ['Tell beforehand', 'Tell after (announcement)', 'Invite a few to attend', 'Private — share photos later'], default: 'Tell after (announcement)', when: 'T-60d', weight: 'med', reversibility: 'costly', emotionalWeight: 'high', difmCapable: 'needs-host', priorityBasis: { rationale: 'How and when family hears is the emotional core of an elopement — an intentional, hard-to-walk-back choice only the couple can own.', tier: 'reasoned' }, why: 'The emotional core of an elopement. Decide intentionally: who hears before, who hears after, and how — so the choice feels owned, not like an accident someone discovers.' },
    { id: 'photography', ask: 'Hire an elopement photographer, or is a friend shooting it?', label: 'Photographer / how you keep the day', options: ['Hire elopement photographer', 'Hire photo + video', 'Friend with a camera', 'Self/timer + phone'], default: 'Hire elopement photographer', when: 'T-75d', blocks: ['photographer'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'With no guests and no reception, the photos ARE the event afterward — the one spend that lasts, and the couple decides how much it is worth.', tier: 'reasoned' }, why: 'With no guests and no reception, the photos ARE the event afterward — the #1 spend and the only thing that lasts. Elopement photographers book out months ahead and often double as the day-of guide.' },
  ],
  ...
  tasks: [
    ...
    // ── THE ORDER ACT THE PURCHASE ROW COULD ONLY MENTION (2026-09-23) ──────
    // `p_rings` has said "Order/size rings weeks ahead" since it was written,
    // and NOTHING covered it: every other pre-event act in this playbook is a
    // task, and the rings had none. Its `buyAt: 'T-3d'` is a PACK-IT date — the
    // note is explicit that the day-of job is packing them — so on the only
    // dated surface a host reads, a $100-$2,000 essential with a six-week
    // production queue looked like a three-day errand.
    //
    // T-56d is eight weeks, the conservative end of a range two independent
    // jewellers converge on (see knowledge/timingProvenance.js#rings, both read
    // in full 2026-09-23). Eight rather than six because this is a START-BY
    // date and because an elopement is usually travelled to — the rings have to
    // be in hand before the trip, not before the ceremony.
    { id: 't_rings', milestoneId: 'elope_book_photographer', phase: 'planning', label: 'Order and size the rings — allow 6–8 weeks for making, engraving and any resize', when: 'T-56d' },
  ...
  purchases: [
    { id: 'p_champagne', item: 'Champagne / sparkling for the toast', category: 'beverage', qtyFlat: 1, unit: 'bottle', where: ['Liquor store', 'Grocery', 'Lodging concierge'], unitCostRange: [18, 60], essential: false, buyAt: 'T-1d', note: 'One 750ml bottle pours ~5–6 toast glasses — plenty for the couple alone or a few witnesses. Add a second bottle near the high end (~10+ people). Confirm alcohol is allowed at the site (many parks restrict it).' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['costhelper-champagne-2026', 'sparklingwine-band-2026'], lastVerified: '2026-08-16', claim: 'Sparkling wine 2026 runs $15-25 a 750ml bottle on average: budget California sparkling near $5 and Spanish cava low, mid-range about $14-25, true Champagne above that.', sufficientWhen: 'Current shelf prices for one budget sparkling and one mid-range bottle confirm the per-bottle range.' } },
    { id: 'p_meal', item: 'Small celebratory meal (picnic, reservation, or to-go)', category: 'food', qtyPerGuest: 1, unit: 'meal', where: ['Restaurant', 'Caterer', 'Grocery / deli'], unitCostRange: [25, 120], essential: true, buyAt: 'T-1d', note: 'No reception — this is the celebration. A reservation for two, a curated picnic, or a private chef all work. Reserve ahead if dining out.' , alternatives: ['Picnic from local deli or grocery — romantic, budget-friendly', 'Farmers market spread — seasonal, meaningful, often cheaper than restaurant'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['boxedlunch-restaurant-2026', 'deli-prepared-meat-2026'], lastVerified: '2026-08-18', claim: 'A small celebratory meal for two, across the three forms this row names. TO-GO or PICNIC: a prepared boxed meal is $12-18.95 a box, so two is $24-38. RESTAURANT: a three-course mid-range meal for two averages $80 nationally with a range of $50-150, before drinks and tip. A deli tray for a slightly larger group is $89.99-119.99. So the band runs from two boxed lunches at its floor to a full restaurant dinner at its ceiling. Tip and tax are not included in the restaurant figure.', sufficientWhen: 'One boxed-lunch price and one three-course restaurant check for two confirm both ends.' }, },
    { id: 'p_bouquet', item: 'Bouquet / boutonniere / simple florals', category: 'decor', qtyFlat: 1, unit: 'set', where: ['Florist', 'Grocery', 'Farmers market'], unitCostRange: [40, 150], essential: false, buyAt: 'T-1d', note: 'The one styling element that reads in every photo. Keep it small and packable for travel.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['paperlust-centerpieces-2026', 'linen-rental-2026'], lastVerified: '2026-08-18', claim: 'Wedding florals for a small ceremony. Per-table florals run $20-50 DIY simple, $50-100 DIY elevated, $75-150 florist standard, $150-300 florist premium; vessel rentals are $8-25 a piece and florist delivery and setup often $200-500 an event, which is EXCLUDED here because an elopement bouquet is collected rather than installed. A bouquet with a boutonniere sits between the DIY-elevated and florist-standard tiers, which is this band.', sufficientWhen: 'One florist quote for a bouquet and boutonniere, collected rather than delivered, confirms the band.' }, },
    { id: 'p_rings', item: 'Wedding rings', category: 'logistics', qtyFlat: 2, unit: 'rings', where: ['Jeweler', 'Online'], unitCostRange: [100, 2000], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN day-of: pack the rings and the license folder FIRST. This date is the PACK-IT date — ordering and sizing is its own task about eight weeks out. Carry them on your person when traveling, never checked.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['weddingrings-survey-2026', 'proposal-items-2026'], lastVerified: '2026-08-18', claim: 'Wedding rings for two, and this band prices BANDS, not an engagement ring. A plain metal band with no stones is $150-700 per ring, so a pair is $300-1,400 - which sits inside this band. Mens bands run $400-1,000 and womens $600-1,800 as published ranges, and a survey line for wedding rings is $3,000 without stating whether that is a pair. AN ENGAGEMENT RING WITH A CENTRE STONE IS A DIFFERENT PURCHASE at a $4,600 survey average and is excluded; folding it in would misprice this row several times over.', sufficientWhen: 'Two current plain-band prices at one jeweller, summed, confirm the band.' }, },
    { id: 'p_keepsakes', item: 'Vow books, marriage license folder, ring box', category: 'logistics', qtyFlat: 1, unit: 'kit', where: ['Online', 'Stationery store'], unitCostRange: [15, 50], essential: false, buyAt: 'T-3d', note: 'Vow books photograph beautifully and keep written vows safe. A document folder protects the license in a pack/pocket.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['weddingstationery-2026', 'proposal-items-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced keepsakes. VOW BOOKS: $5.00 a 2-book set at a party retailer, $12.99 at a bookseller. RING BOX: $12.99-17.99 for a slim model. A MARRIAGE LICENCE HOLDER is NOT priced here - one retailer at $35.00 and one marketplace listing of a different product form at $14.95 was not enough to establish it - so that component rests on the other two, which is why confidence is low. Books and a box alone sum to about $18-31.', sufficientWhen: 'A licence-holder price from a second retailer would complete this band; the books and box are already confirmed.' }, },
    { id: 'p_glasses', item: 'Two toast glasses (packable/unbreakable)', category: 'rental', qtyFlat: 2, unit: 'glasses', where: ['Online', 'Outdoor store', 'Home'], unitCostRange: [10, 40], essential: false, buyAt: 'T-3d', note: 'Bring your own — remote sites have nothing. Unbreakable/stemless travels best.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['glassware-disposable-2026', 'glassware-rental-2026'], lastVerified: '2026-08-18', claim: 'Two toast glasses, packable. Disposable plastic flutes are $0.28 each at foodservice and $0.61-1.00 at party retail - far below this band, which prices a KEEPABLE pair rather than disposables. Rented crystal is $1.25-2.99 a glass. A bought unbreakable pair for an elopement sits above both, which is what this band covers.', sufficientWhen: 'A retail price for a packable unbreakable flute pair confirms the band directly.' }, },
    { id: 'p_packout', item: 'Trash bag + reusable tote for leave-no-trace pack-out', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Outdoor store'], unitCostRange: [5, 15], essential: true, buyAt: 'T-1d', note: 'Pack out every bottle, wrapper, and petal — leave-no-trace is required at most natural sites and is often a permit condition.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['cleanup-heavyduty-2026', 'homesetup-retail-2026'], lastVerified: '2026-08-18', claim: 'Leave-no-trace pack-out, summed. A true 3-mil contractor bag is $0.62-1.15 each depending on size and case count - the right bag for carrying out rather than a kitchen liner. A reusable tote is $0.89-0.99 basic or $1.99-5.00 for a heavier one. A few bags plus a sturdy tote lands in this band.', sufficientWhen: 'A contractor-bag case price divided per bag, plus one tote, confirm the band.' }, },
  ],
