...
const dinnerParty = {
  ...
  decisions: [
    { id: 'format', label: 'Seated dinner or buffet / family-style?', options: ['Plated 3-course', 'Family-style', 'Buffet', 'Grazing + small plates'], default: 'Family-style', defaultWhy: 'Family-style is the default because platters going on the table mean you serve once and then sit down, instead of plating every course while the table waits. Change it if the menu is built around dishes that have to leave the kitchen already plated.', when: 'T-21d', blocks: ['menu', 'rentals', 'seating'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Plated versus family-style versus buffet sets your plate counts, the table layout, and how much cooking lands on you mid-party — but it stays easy to change early.', tier: 'reasoned' }, why: 'Drives plate/serveware counts, table layout, and how much active cooking lands on the host during the party.' },
    { id: 'menu', ask: 'What are you cooking, and what’s the vegetarian main?', label: 'Lock the menu (incl. a vegetarian main)', options: [], default: null, when: 'T-14d', dependsOn: ['format', 'dietary'], blocks: ['shopping_list', 'cook_schedule'], deliversHeartMoment: true, weight: 'high', reversibility: 'reversible', emotionalWeight: 'high', difmCapable: 'needs-host', priorityBasis: { rationale: 'This is the meal itself — every quantity and the cook timeline derive from it, and the dish landing quiet at the table is the whole reason people came.', tier: 'reasoned' }, why: 'Every quantity, the shopping list, and the cook timeline derive from this. A dinner party with no locked menu at a week out is the #1 host failure mode.' },
    { id: 'dietary', ask: 'Any allergies, vegetarians, or diets to cook around?', label: 'Dietary needs and allergies', timingCategory: 'invitation', options: ['Vegetarian', 'Vegan', 'Gluten-free', 'Nut allergy', 'Dairy-free', 'Shellfish', 'Halal', 'Kosher', 'Pescatarian', 'Alcohol-free'], default: null, when: 'T-14d', blocks: ['menu'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'One unflagged severe allergy can send a guest to the ER, so allergies must be collected before the menu locks — and only the guests can answer.', tier: 'reasoned' }, why: 'One unflagged severe allergy can send a guest to the ER. Must be collected BEFORE the menu locks, never after.' },
    { id: 'alcohol', ask: 'Full bar, wine and a cocktail, BYOB, or zero-proof?', label: 'Alcohol strategy', options: ['Host provides full bar', 'Wine + one cocktail', 'Wine + beer only', 'BYOB', 'Zero-proof / dry'], default: 'Wine + one cocktail'  /* was 'Wine + one signature cocktail' — not an option; see note below */, defaultWhy: 'Wine plus one cocktail is the default because it gives the table something to arrive with and something to sit down with, without the bottles and glassware a full bar needs. Change it if your table does not drink, or if you would rather ask people to bring what they want.', when: 'T-14d', blocks: ['beverage_purchases'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Alcohol is often a quarter to a third of a dinner-party budget and sets glassware counts, but a wine-plus-one-cocktail default is safe and easy to adjust at the store.', tier: 'reasoned' }, why: 'Determines spend (often 25-40% of a dinner-party budget), glassware counts, and whether a bartender is worth it.' },
    { id: 'seating', label: 'Open seating or place cards?', options: ['Open', 'Place cards', 'Host-assigned'], default: 'Host-assigned', recommendedWhen: [{ when: { guests: { lt: 8 } }, pick: 'Open', because: 'under eight, the shuffle sorts itself out' }, { when: { guests: { gte: 8 } }, pick: 'Host-assigned', because: 'at eight or more, assigned seats spare everyone the shuffle' }], defaultWhy: 'Host-assigned is the default because this playbook plans for a table of about eight, and at that size deciding the seats yourself spares everyone the shuffle. Change it if you are down to a smaller table of close friends who will sort themselves out.', when: 'T-2d', dependsOn: ['format'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'For eight or more, assigned seats just spare everyone the awkward shuffle — a low-stakes call the host can set the night before.', tier: 'reasoned' }, why: 'For 8+ guests, assigned seating prevents the awkward shuffle and lets the host seat the right energy together.' },
    { id: 'help', label: 'DIY, or bring in help?', options: ['Fully DIY', 'Hire a cleaner for after', 'Hire a bartender', 'Drop-off catering for 1 course', 'Private chef'], default: 'Fully DIY', defaultWhy: 'Fully DIY is the default because a dinner party this size is one person\'s job, and every hire has to earn its cost. Change it if you would rather be at the table than at the sink — a post-party cleaner or a bartender is what buys that back.', when: 'T-10d', blocks: ['vendors'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'A cleaner or a bartender is the honest tradeoff that lets you enjoy your own party — worth deciding, and easy to add or drop.', tier: 'reasoned' }, why: 'The honest tradeoff: a $150 post-party cleaner or a $200 bartender is what lets the host actually enjoy their own party.',
      // "a dinner party this size is one person's job" is a FACT CLAIM about the host, and it
      // was unconditional — it printed unchanged to a host who had set hostCapacity:'has_help'
      // AND named people on their own food/timeline rows. TWO entries because a `when` clause
      // is an AND: either statement of "I have help" is enough on its own. Both facts are
      // unknown on an event that has said nothing, an unknown fact refuses its clause, so the
      // authored line above still stands for everyone else. (recommendedPick.resolveCopy)
      copyWhen: [
        { when: { hostCapacity: { eq: 'has_help' } }, defaultWhy: 'Fully DIY is the default because a dinner party this size is within reach of the people already on it, and every hire has to earn its cost. Change it if you would rather be at the table than at the sink — a post-party cleaner or a bartender is what buys that back.' },
        { when: { helperCount: { gte: 1 } }, defaultWhy: 'Fully DIY is the default because a dinner party this size is within reach of the people already on it, and every hire has to earn its cost. Change it if you would rather be at the table than at the sink — a post-party cleaner or a bartender is what buys that back.' },
      ] },
  ],
