...
const wedding = {
  ...
  decisions: [
    { id: 'budget', ask: 'What’s the number, and who is paying it?', label: 'Total budget + who pays', options: ['Under $25k', '$25k–$50k', '$50k–$100k', '$100k+'], default: '$25k–$50k', when: 'T-365d', blocks: ['venue', 'guestcount', 'vendor_team', 'catering_style'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'The number every other choice back-solves from — it sets the guest-count ceiling and which vendors are even reachable, and who-pays makes it a delicate family conversation.', tier: 'reasoned' }, why: 'The first gate. Budget sets the guest-count ceiling and which vendors are reachable — every later decision back-solves from this number.' },
    { id: 'guestcount', ask: 'How many are you inviting — 50, 120, or 250?', label: 'Guest-count target (the budget gate)', options: ['Intimate (≤50)', 'Mid (50–120)', 'Large (120–250)', 'Grand (250+)'], default: 'Mid (50–120)', when: 'T-365d', dependsOn: ['budget'], blocks: ['venue', 'catering_style', 'bar'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'Headcount times per-head is the single biggest cost lever, and deciding who makes the list — and who gets cut — is personal, so the couple has to own it.', tier: 'reasoned' }, why: 'Headcount × per-head is the single biggest cost lever. Set the target BEFORE touring venues — a venue that fits 80 won\'t fit 200, and per-head catering/bar scale directly off it.' },
    { id: 'venue', ask: 'Where are you getting married, and on what date?', label: 'Venue + date (book FIRST)', options: ['Full-service venue (in-house catering)', 'Raw / blank-canvas venue (bring everything)', 'Hotel / resort', 'Private estate / backyard', 'Destination'], default: 'Full-service venue (in-house catering)', when: 'T-365d', blocks: ['vendor_team', 'catering_style', 'rain_plan'], weight: 'high', reversibility: 'locked', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'The date and place are the hardest things to get and every other vendor\'s availability anchors to them — once booked on a signed deposit, it does not move.', tier: 'reasoned' }, why: 'The venue and date are booked first because they\'re the hardest to get and they anchor every other vendor\'s availability. A full-service venue folds in catering/rentals; a blank-canvas venue means you source it all.' },
    { id: 'vision', ask: 'Black-tie, garden romantic, modern, rustic, or traditional?', label: 'Style + formality (the design brief)', options: ['Black-tie / formal', 'Garden / romantic', 'Modern / minimal', 'Rustic / barn', 'Cultural / traditional'], default: 'Garden / romantic', when: 'T-330d', dependsOn: ['venue'], blocks: ['vendor_team'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'The design language that briefs every creative vendor — an expression of the couple\'s taste, but a direction you can still reshape before the contracts lock.', tier: 'reasoned' }, why: 'The design language that briefs the florist, stationery, attire, and rentals so the whole event reads as one idea instead of a pile of pretty things.' },
    { id: 'vendor_team', ask: 'A full planner, a month-of coordinator, or family?', label: 'The vendor team (lock the year-out ones)', options: ['Planner-led (full)', 'Month-of coordinator', 'Venue coordinator only', 'DIY / family'], default: 'Month-of coordinator', defaultWhy: 'A month-of coordinator is the default because it is the floor rather than the recommendation: somebody other than the couple has to run the day. Change it up to a full planner if the venue is blank-canvas or the vendor list is long, because that is the year this saves, not the day. Both are a real contract signed early, so price them before the year-out vendors take the rest of the budget.', when: 'T-300d', dependsOn: ['budget', 'venue'], blocks: [], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Photographer, caterer and band book far in advance, so reserving them early is high-stakes — but a month-of coordinator is a safe floor the app can default to.', tier: 'reasoned' }, why: 'Photographer, caterer, planner, and band/DJ book out ~a year — reserve them right after the date. A planner is the force-multiplier; at minimum a month-of coordinator runs the day so the couple doesn\'t.' },
    { id: 'ceremony', label: 'Ceremony type + officiant', ask: 'What kind of ceremony, and who will lead it?', options: ['Religious / house of worship', 'Civil', 'Symbolic / personal', 'Cultural / traditional'], default: 'Symbolic / personal', when: 'T-300d', dependsOn: ['venue'], blocks: ['run_of_show'], weight: 'high', reversibility: 'costly', emotionalWeight: 'high', difmCapable: 'needs-host', deliversHeartMoment: true, priorityBasis: { rationale: 'This is the vows and who stands up to lead them — the reason everyone is in the room; protect it above the logistics that surround it.', tier: 'reasoned' }, why: 'Drives officiant booking, any required premarital steps, ceremony length (20–30 min civil, 45–60 religious), and whether ceremony + reception share a site.' },
    { id: 'catering_style', ask: 'Plated and seated, buffet, family-style, or stations?', label: 'Catering style', options: ['Plated / seated', 'Buffet', 'Family-style', 'Stations / heavy passed'], default: 'Plated / seated', when: 'T-240d', dependsOn: ['guestcount', 'budget'], blocks: ['rentals'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Sets per-head cost, staffing, rental counts, and reception pacing — a real budget lever, but a standard plated dinner is a safe default the app can set.', tier: 'reasoned' }, why: 'Sets per-head cost, staffing, rental counts, and reception pacing — plated runs ~90 min of service, buffet ~45. Tasting and final menu follow.',
      // AUTHORED per-option planner knowledge (grounds the option display — no per-render
      // invention). Each note is the style's real distinguishing tradeoff: what it's good for
      // and the cost. `defaultWhy` explains WHY the app derives Plated as the safe pick.
      optionNotes: {
        'Plated / seated': 'Most formal · seated, coursed service · most staff & place settings',
        'Buffet': 'Casual · guests self-serve · lighter staff · faster (~45 min)',
        'Family-style': 'Warm & shared · platters per table · mid staff, larger tables',
        'Stations / heavy passed': 'High-energy grazing · no seated meal · most rentals & mingling',
      },
      defaultWhy: 'A standard plated dinner is the safe default for a formal wedding — clean per-head costs and controlled pacing. Change it for a more casual, mingling feel.' },
    { id: 'bar', ask: 'Open bar, beer and wine with a signature, cash bar, or dry?', label: 'Bar service', options: ['Open bar (full)', 'Beer + wine + signature', 'Cash bar', 'Dry'], default: 'Beer + wine + signature', defaultWhy: 'Beer, wine and one signature is the default because the bar is often the second-largest line after catering, and this version serves the whole room without opening the full spirits bill. Change it if the venue supplies the alcohol and sells only its own packages, or if a dry reception is the right call for this family — both decide the number long before the volume math does.', when: 'T-180d', dependsOn: ['budget'], blocks: ['bar_purchases'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Often the second-largest line after catering, but beer-plus-wine-plus-a-signature is a safe, adjustable default the app can propose.', tier: 'reasoned' }, why: 'Often the second-largest line after catering. Drives alcohol volume (~1 drink/guest/hr, ~7 over a 5h reception) and whether you supply alcohol or the venue does.' },
    { id: 'music', ask: 'A live band, a DJ, or a playlist you run yourselves?', label: 'Music — band or DJ', options: ['Live band', 'DJ', 'Band ceremony + DJ reception', 'Acoustic / DIY playlist'], default: 'DJ', defaultWhy: 'A DJ is the default because one booking covers the whole reception — entrances, first dance, toasts and the dance floor — at a fraction of a band. Change it if a live band is the point of the night for you, and change it now: bands book about a year out, so deciding late usually decides for the DJ anyway.', when: 'T-330d', dependsOn: ['budget'], blocks: ['run_of_show'], weight: 'med', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Drives the dance-floor energy and the run-of-show cues; a DJ is the safe default, though a live band books far ahead and costs multiples to swap in late.', tier: 'reasoned' }, why: 'Drives the dance-floor energy and the run-of-show cues (entrances, first dance, toasts). Bands book a year out and cost multiples of a DJ.' },
  ],
  ...
  purchases: [
    { id: 'p_favors', item: 'Guest favors', category: 'decor', qtyPerGuest: 1, unit: 'favor', where: ['Etsy', 'Party store', 'Specialty / local'], unitCostRange: [1, 5], essential: false, buyAt: 'T-3d', note: 'Order well ahead if custom; the 3 days out window is for pickup/assembly, not first purchase.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['zola-favors-2026', 'theknot-realweddings-2025'], lastVerified: '2026-08-14', claim: 'US wedding favors run $1-5 per guest, with $3 the typical mid-range spend; 56% of couples keep favors at $5 or less per guest.', sufficientWhen: 'Re-checked against the next Zola registry/gifting survey and The Knot Real Weddings study; pricing policy treats commercial prices as stale after 45 days, so re-verify the band before quoting it as current.' } },
    { id: 'p_welcomebags', item: 'Welcome bags (out-of-town guests)', category: 'logistics', qtyFlat: 1, qtyPer: 4, unit: 'bag', where: ['Amazon', 'Party store', 'Grocery'], unitCostRange: [15, 35], essential: false, buyAt: 'T-3d', note: 'One per hotel room for traveling guests — water, snacks, a map/itinerary card. Assemble the day before.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['paperlust-welcomebags-2026', 'pacificgiftbox-welcomebags-2026'], lastVerified: '2026-08-15', claim: 'Most US couples spend $15-35 per bag on wedding welcome bags; under $15 is achievable for a simple build and curated/assembled boxes start around $50.', sufficientWhen: 'Re-checked against the next welcome-bag cost guides; pricing policy treats commercial prices as stale after 45 days. Note the published bands EXCLUDE assembly labour and hotel delivery coordination.' } },
    { id: 'p_emergencykit', item: 'Day-of emergency kit', category: 'logistics', qtyFlat: 1, unit: 'kit', where: ['Pharmacy', 'Amazon', 'Grocery'], unitCostRange: [20, 45], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN. Sewing kit, safety pins, fashion tape, stain remover, pain reliever, bobby pins, hairspray, blotting papers, deodorant, breath mints, Band-Aids, phone charger, scissors, extra earring backs. Pack at rehearsal.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['myweddingkit-emergency-2026', 'amazon-emergencykit-2026'], lastVerified: '2026-08-15', claim: 'A day-of emergency kit costs $20-45: ~$25-40 to build from drugstore items, or $13-35 for a pre-assembled retail kit. Boutique pre-made kits run $60-100 and are the exception, not the band.', sufficientWhen: 'Re-priced against drugstore and marketplace listings; commercial prices are stale after 45 days under the pricing policy.' } },
    { id: 'p_signage', item: 'Signage (welcome sign, seating chart, directional)', category: 'decor', qtyFlat: 4, unit: 'sign', where: ['Etsy', 'Print shop', 'Sign maker'], unitCostRange: [25, 120], essential: false, buyAt: 'T-3d', note: 'Welcome sign, seating chart, bar/menu, and directional signs — proofread names against the FINAL list.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['printitmyway-signs-2026', 'weddingsigns-materials-2026'], lastVerified: '2026-08-15', claim: 'Wedding signage spans $25-120 for a typical welcome sign: foam board $12-25, wood $22-48, acrylic $28-65 by material, with finished welcome signs $75-145. Acrylic seating charts run higher, $125-185.', sufficientWhen: 'Re-checked against sign-maker price lists; note a full acrylic seating chart sits ABOVE this band and should be priced separately if the couple wants one.' } },
    { id: 'p_guestbook', item: 'Guest book + pens', category: 'decor', qtyFlat: 1, unit: 'set', where: ['Etsy', 'Bookstore', 'Party store'], unitCostRange: [15, 50], essential: false, buyAt: 'T-3d' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['costhelper-guestbook-2026', 'mixbook-guestbook-2026'], lastVerified: '2026-08-15', claim: 'Wedding guest books run $15-50 for basic through moderately priced books; luxury leather or gold-foil editions reach $100-200 and audio/digital guest books are priced separately.', sufficientWhen: 'Re-checked against retail guest-book pricing; the band deliberately covers basic-to-moderate, not luxury or digital alternatives.' } },
    { id: 'p_toast_champagne', item: 'Toast champagne / sparkling (if self-supplied)', category: 'beverage', qtyPerGuest: 0.14, unit: 'bottle', where: ['Liquor store', 'Costco', 'Warehouse'], unitCostRange: [12, 35], essential: false, buyAt: 'T-3d', note: '~1 bottle per 6–8 guests for a half-glass toast. Many full-service venues/caterers supply this — only buy if your bar is self-supplied.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['costhelper-champagne-2026', 'sparklingwine-band-2026'], lastVerified: '2026-08-15', claim: 'A self-supplied toast bottle runs $12-35: sparkling wine averages $15-25 (budget $10-15), Spanish cava ~$14, with true Champagne starting $30-40 and rising steeply.', sufficientWhen: 'Re-checked against retail sparkling/Champagne pricing. Excludes venue corkage and per-bottle corking fees, which are charged separately and can exceed the bottle.' } },
    { id: 'p_bar_alcohol', item: 'Reception alcohol (if self-supplied bar)', category: 'beverage', qtyPerGuest: 7, unit: 'drinks', where: ['Liquor store', 'Costco', 'Warehouse'], unitCostRange: [2, 6], essential: false, buyAt: 'T-3d', note: 'Only if you supply the bar. US norm ~1 drink/guest/hr (≈2 first hour, 1 each after) → ~7 drinks/guest over a 5h reception. Split ~50% beer/wine, ~30% spirits, ~20% non-alcoholic. Confirm corkage/licensing with the venue.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['selfsupplied-bar-2026', 'costhelper-champagne-2026'], lastVerified: '2026-08-15', claim: 'Self-supplied reception alcohol costs $2-6 per drink, derived from a self-supply spend of $15-30 per person across the planning norm of ~5-7 drinks per guest; a venue open bar is $45-100 per person instead.', sufficientWhen: 'Re-checked against open-bar and self-supply cost guides. EXCLUDES corkage, which venues allowing BYO almost always charge ($1.50-3 per beer, $15-25 per bottle of wine or spirits) and which can exceed the alcohol itself.' } },
  ],
  ...
  risks: [
    ...
    { id: 'r_count', trigger: 'Final headcount wrong or late to the caterer', severity: 'high', mitigation: 'Lock RSVPs, chase non-responders, and give the caterer the FINAL count at 2 weeks out — most contracts bill off it and won\'t go down after.' },
    { id: 'r_vendor_payment', trigger: 'Outstanding vendor balances on the day', severity: 'high', mitigation: 'Pay all final balances by 2 weeks out; pre-stage labeled tip envelopes and assign one person to hand them out so the couple never touches money.' },
    { id: 'r_gift_theft', trigger: 'Gift table or card box walks off at a big reception', severity: 'medium', mitigation: 'The insurance-industry pattern (Markel/III): name one trusted gift attendant, use a locked card box, keep the table in a populated corner away from exits, and ship registry gifts home. Gifts are also an insurable rider on event policies.' },
    { id: 'r_vendor_noshow', trigger: 'A key vendor cancels, is double-booked, or no-shows', severity: 'high', mitigation: 'Sign contracts (not verbal holds), confirm arrival times at a week out, keep a backup officiant/DJ contact, and verify vendor COI where the venue requires it.' },
    { id: 'r_timeline', trigger: 'No run-of-show / hair + makeup runs long', severity: 'med', mitigation: 'Build an hour-by-hour timeline at 2 months out, pad hair/makeup (start early, ~45 min/face + bride buffer), and have the coordinator hold the schedule on the day.' },
    { id: 'r_seating', trigger: 'Seating chart unresolved as RSVPs trickle in', severity: 'med', mitigation: 'Draft seating at 2 months out, finalize after the headcount lock, and proofread escort cards + signage against the final list.' },
