...
const retirementParty = {
  ...
  // Each priority-bearing decision carries a `priorityBasis` — the honest provenance of
  // its weight/reversibility/emotionalWeight (DECISION_SCHEMA_SPEC §4.A; shape documented
  // above crabFeast's decisions block). `weight` is an editorial judgment, so its grounding
  ...
  decisions: [
    ...
    { id: 'bar', label: 'Bar strategy + the honoree\'s favorite drink', options: ['Beer + wine + honoree\'s favorite', 'Full open bar', 'Beer + wine only', 'Beer/wine + a signature cocktail', 'Limited / cash bar'], default: 'Beer + wine + honoree\'s favorite', when: 'T-25d', blocks: ['beverage_purchases', 'glassware'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'Stocking the honoree\'s own drink is what makes the bar feel like them — meaningful, but easily adjusted at the store.', tier: 'reasoned' }, why: 'Stocking the honoree\'s actual favorite drink (their go-to scotch, a particular wine, a regional beer) is the cheapest way to make the bar feel like THEM. It also fixes spirit/mixer quantities and whether a bartender is worth it.' },
    { id: 'tribute', label: 'Speeches / tribute format', options: ['3-5 pre-assigned speakers', 'MC + open mic (capped)', 'Slideshow/video + a few toasts', 'Roast-style (lighthearted)', 'Single heartfelt toast + card'], default: '3-5 pre-assigned speakers', when: 'T-14d', blocks: ['runofshow', 'av'], deliversHeartMoment: true, weight: 'high', emotionalWeight: 'high', reversibility: 'costly', difmCapable: 'needs-host', priorityBasis: { rationale: 'This is the moment the guest of honor remembers — protect it above logistics.', tier: 'reasoned' }, why: 'The tribute is the hero beat. Pre-assigning 3-5 speakers (boss, peer, family, a surprise mentor) at ~2-3 min each keeps it warm and on-time. Decide here whether you need a mic and a screen for a slideshow.' },
    { id: 'music', label: 'Music for the room — DJ or a curated playlist?', options: ['Curated playlist + a good speaker', 'Hire a DJ', 'Live musician (era-appropriate)', 'No music / ambient only'], default: 'Curated playlist + a good speaker', when: 'T-14d', blocks: ['vendors'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Background mood only, free to swap, and a playlist is always fine — the lowest-stakes call on the board.', tier: 'reasoned' }, why: 'Sets the mood before and after the tribute and covers any dancing once the speeches wrap. A DJ can also MC the room and cue music around the tribute beats; a curated playlist through a real speaker is the free, still-warm alternative for a smaller room.' },
    { id: 'invite', label: 'Invite + RSVP + dietary/allergy + accessibility ask', options: ['Paper invite', 'Digital (Paperless Post / Evite)', 'Email (for coworkers)', 'Phone + email combo'], default: 'Digital (Paperless Post / Evite)', when: 'T-30d', blocks: ['headcount', 'accessibility'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'Headcount is the master variable every other quantity scales off — high-stakes, and hard to redo once it is sent.', tier: 'reasoned' }, why: 'Headcount is the master variable — every food, drink, and seat quantity scales off it. Collect allergies AND mobility/hearing needs here, before menu and room layout lock; an older guest list makes this non-optional.' },
    { id: 'help', standsDownWhen: { id: 'venue', in: ['Restaurant private room'] }, label: 'DIY, or bring in help?', options: ['Fully DIY', 'Hire a bartender', 'Drop-off / buffet catering', 'Bartender + cleaner', 'Day-of helper for refills + plating'], default: 'Drop-off / buffet catering', when: 'T-20d', blocks: ['vendors'], weight: 'med', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Whether one host can run food, bar, and program at once decides if the night works — a real, costly-to-reverse staffing call the app can default.', tier: 'reasoned' }, why: 'For 40-50 mostly-older guests one host cannot run a buffet, tend bar, AND run the program. Drop-off catering or a bartender (~$250) is what lets the host actually orchestrate the surprise and the speeches.' },
  ],
  ...
  tasks: [
    ...
    { id: 't_arrival', milestoneId: 'event', phase: 'program', label: 'Guests in and settled by the arrive-by time; greet the honoree when they arrive', copyByAnswer: { surprise: { 'Full surprise': 'SURPRISE choreography: guests in + quiet by the arrive-by time; co-conspirator brings the honoree in on cue', 'Soft surprise (honoree knows, not the details)': 'Guests in and settled by the arrive-by time; co-conspirator times the honoree\'s arrival so the room is ready' } }, when: 'T0 -10m' },
  ...
  purchases: [
    { id: 'p_buffet_protein', item: 'Carved meats & chicken (plus a vegetarian main)', category: 'food', qtyPerGuest: 0.5, unit: 'lb', where: ['Caterer', 'Costco', 'Grocery', 'Butcher'], unitCostRange: [4, 10], essential: true, buyAt: 'T-1d', note: 'For a buffet at a meal hour. If going heavy-apps instead, swap this for ~10-12 bites/guest.', provenance: { tier: 'researched', confidence: 'med', verificationStatus: 'researched', sources: ['webstaurant-protein-2026'], note: 'Grounded to webstaurant-protein-2026: ~0.5 lb cooked protein/guest (~8 oz) sits at the source-stated buffet portion of ~6–8 oz.', claim: '~0.5 lb cooked protein per adult guest and ~1-1.25 servings/guest overall is the standard US buffet planning heuristic at a meal hour', sufficientWhen: 'US catering industry data from ≥2 sources confirms the ~0.5 lb/guest cooked-protein norm for a meal-hour buffet' }, alternatives: ['Roast chicken — cheaper than carved beef or salmon', 'Costco deli trays — budget option, no cooking required', 'Pasta buffet main — cheapest per head, crowd-pleasing'] },
    { id: 'p_sides', item: 'Salads & potatoes (rolls, a hot veg)', category: 'food', qtyPerGuest: 0.5, unit: 'lb', where: ['Caterer', 'Grocery', 'Costco'], unitCostRange: [2, 5], essential: true, buyAt: 'T-1d', note: '2-3 sides for 25-40 guests; 3+ for 50. Sides stretch the protein and cover the vegetarians.', alternatives: ['Costco pre-made sides — cheaper, still presentable', 'Grocery deli salads — if cooking time is short'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_apps', item: 'Cheese & charcuterie (dips, easy bites)', category: 'food', qtyPerGuest: 4, unit: 'bites', where: ['Grocery', 'Costco', 'Caterer', 'Trader Joe\'s'], unitCostRange: [0.6, 1.8], essential: true, buyAt: 'T-1d', note: 'Cocktail-hour bites BEFORE the buffet opens so early arrivals (and the surprise window) have something to graze.', provenance: { tier: 'heuristic', confidence: 'med', verificationStatus: 'synthesized', note: 'A few starter bites/guest cover the pre-meal window; a full heavy-apps format would target ~10-12 bites/guest.', claim: 'A cocktail-hour appetizer window needs only a few bites per guest; a heavy-apps format replacing a full meal targets ~10-12 bites per guest', sufficientWhen: 'US event-catering guides from ≥2 sources confirm the ~10-12 bites/guest threshold for a heavy-apps-as-meal format' }, alternatives: ['Costco/Trader Joe\'s frozen apps — budget, bake day-of', 'Cheese + cracker tray from deli — no prep, cheaper per bite'] },
    { id: 'p_cake', item: 'Retirement cake or dessert table ("Happy Retirement" / years-of-service)', category: 'food', qtyPer: 13, qtyFlat: 1, unit: 'cake', where: ['Bakery', 'Grocery', 'Costco'], unitCostRange: [25, 55], essential: true, buyAt: 'T-1d', note: 'A named cake is the expected centerpiece of the cut-and-toast moment; order it personalized 1 week ahead.', alternatives: ['Costco half-sheet cake — add custom message, cheapest per slice', 'Grocery bakery sheet cake — cheaper than custom bakery, personalize day-of'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_champagne', item: 'Champagne / sparkling wine for the toast', category: 'beverage', qtyFlat: 1, qtyPer: 7, unit: 'bottle per ~7 guests', where: ['Wine shop', 'Total Wine', 'Costco'], unitCostRange: [12, 30], essential: true, buyAt: 'T-3d', note: 'Toast pour is a half-glass, so ~1 bottle per 6-8 guests covers everyone. Buy a couple extra; a sparkling-cider option lets non-drinkers toast too.', provenance: { tier: 'heuristic', confidence: 'high', verificationStatus: 'synthesized', note: 'Champagne-toast heuristic: a 750ml bottle pours ~6-8 toast portions (half-glass each); plan ~1 bottle per 6-8 guests.', claim: 'A 750ml champagne bottle pours ~6-8 half-glass toast portions; ~1 bottle per 6-8 guests covers a single toast', sufficientWhen: 'pour math confirmed (750ml ÷ ~100ml half-glass = ~7 portions); aligned against ≥1 bartending or catering reference' } },
    { id: 'p_wine', item: 'Still wine (mix of white/red — lean to what the crowd drinks)', category: 'beverage', qtyPerGuest: 0.4, unit: 'bottle', where: ['Wine shop', 'Total Wine', 'Grocery'], unitCostRange: [10, 22], essential: true, buyAt: 'T-3d', note: 'Wine + beer carry most of the ~1 drink/guest/hour load. An older daytime crowd often skews wine-heavy.', provenance: { tier: 'researched', confidence: 'med', verificationStatus: 'researched', sources: ['bar-provision-2026'], note: 'DERIVATION CORRECTED 2026-08-01 (Phase 5C.1). Grounded to bar-provision-2026, which states ~1/2 bottle of wine per DRINKING guest (~1 bottle per ~2.5 drinking guests per hour; a 750ml pours ~5 servings). This line is expressed per GUEST, not per drinker, so the sourced 0.5/drinker is multiplied by an assumed ~80% drinking rate for an older mixed daytime crowd: 0.5 x 0.8 = 0.4 bottle/guest. The ~80% drinker rate is an ASSUMPTION, not sourced — it is the one unverified step in this derivation. Sibling check: Anniversary and Vow Renewal both carry 0.5 with the same source; their claims are stated per DRINKING guest while their qtyPerGuest applies to all guests, so the three lines differ in unit convention rather than in substance. PRIOR CLAIM WITHDRAWN: the earlier text asserted "wine carries ~40% of the drink load". That was wrong twice — the source assigns ~40% to BEER (beer+wine ~75%, implying wine ~35%), and the stated arithmetic (3h x 1 drink/h x 40% / 5 glasses) yields 0.24 bottle/guest, not the 0.4 published. The VALUE survives; only the reasoning was defective.', claim: 'Source states ~1/2 bottle of wine per drinking guest at ~1 drink/guest/hour; at an assumed ~80% drinking rate for an older daytime crowd this yields ~0.4 bottle per guest', sufficientWhen: 'A drinking-participation rate for older daytime retirement gatherings confirms the ~80% assumption, OR wine consumption data from >=2 comparable 3h events confirms ~0.4 bottle/guest when beer/seltzer is also present' } },
    { id: 'p_beer', item: 'Beer + hard seltzer (cans/bottles, on ice)', category: 'beverage', qtyPerGuest: 1.5, unit: 'cans', where: ['Grocery', 'Costco', 'Liquor store'], unitCostRange: [1.2, 2.5], essential: true, buyAt: 'T-3d', provenance: { tier: 'heuristic', confidence: 'med', verificationStatus: 'synthesized', note: 'Plan ~1 drink/guest/hour; beer/seltzer is the easy self-serve share of the mix over a 3h event.', claim: 'Beer and seltzer carry the self-serve portion of ~1 drink/guest/hour; ~1.5 cans/guest covers the beer share over 3 hours', sufficientWhen: 'consumption data from comparable 3h retirement-style parties confirms ~1.5 beer/seltzer units/guest when wine is also present' } },
    { id: 'p_fav_drink', item: "The honoree's favorite drink (their go-to spirit / wine / regional beer)", category: 'beverage', qtyFlat: 2, unit: 'bottles', where: ['Liquor store', 'Wine shop'], unitCostRange: [20, 70], essential: true, buyAt: 'T-3d', note: 'The signature personal touch — stock what THEY actually drink, plus mixers/garnish if it\'s a cocktail. Toast with it.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_nonalc', item: 'Zero-proof: sparkling cider, sodas, iced tea, sparkling + flat water, juice', category: 'beverage', qtyPerGuest: 2.5, unit: 'drinks', where: ['Grocery', 'Costco'], unitCostRange: [0.6, 1.5], essential: true, buyAt: 'T-3d', note: 'An older, mixed daytime crowd drinks a LOT of non-alcoholic — over-provision water, iced tea, and a toast-able sparkling cider.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_coffee', item: 'Coffee + tea service (with cream, sugar, decaf)', category: 'beverage', qtyPerGuest: 1, unit: 'cups', where: ['Grocery', 'Costco'], unitCostRange: [0.3, 0.8], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: an older crowd expects coffee, especially with cake — set up a self-serve coffee/tea station.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_ice', item: 'Ice (for chilling + drinks)', category: 'beverage', qtyPerGuest: 1.5, unit: 'lb', where: ['Grocery', 'Gas station'], unitCostRange: [0.2, 0.4], essential: true, buyAt: 'T0', note: 'COMMONLY FORGOTTEN. ~1.5 lb/guest covers chilling bottles AND drink ice; add 15-20% for melt.', provenance: { tier: 'researched', confidence: 'med', verificationStatus: 'researched', sources: ['bar-provision-2026'], note: 'Grounded to bar-provision-2026: ~1.5 lb ice/guest is the source-stated ice provisioning rate.', claim: '~1.5 lb of ice per guest covers both bottle chilling and drink service at an indoor 3-hour event with a bar', sufficientWhen: 'US party-planning guides from ≥2 sources confirm the ~1-1.5 lb ice/guest range for an indoor bar event' } },
    { id: 'p_memory_display', item: 'Memory/photo display: prints, frames or a foam board, easel, timeline of career', category: 'decor', qtyFlat: 1, unit: 'kit', where: ['Drugstore photo', 'Amazon', 'Craft store', 'Office store'], unitCostRange: [25, 80], essential: true, buyAt: 'T-3d', note: 'The heart of the room. A photo wall / "memory lane" timeline is the #1 conversation-starter at a retirement party.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_card', item: 'Oversized signing card / guest book / memory book + pens', category: 'decor', qtyFlat: 1, unit: 'set', where: ['Card shop', 'Amazon', 'Etsy', 'Office store'], unitCostRange: [10, 35], essential: true, buyAt: 'T-3d', note: 'A jumbo card or memory book everyone signs becomes the keepsake the retiree takes home — set it out early with pens.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_decor', item: 'Retirement decor (banner, balloons, "Happy Retirement" sign, table centerpieces, candles)', category: 'decor', qtyFlat: 1, unit: 'kit', where: ['Party store', 'Amazon', 'Etsy'], unitCostRange: [25, 70], essential: false, buyAt: 'T-3d', note: 'One strong banner + a few centerpieces beats scattered clutter; lean to the honoree\'s career/hobby theme.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_flowers', item: 'Flowers (a centerpiece + a bouquet to present to the honoree)', category: 'decor', qtyFlat: 1, qtyPer: 15, unit: 'arrangement per ~15 guests', where: ['Florist', 'Trader Joe\'s', 'Grocery'], unitCostRange: [15, 45], essential: false, buyAt: 'T-1d', note: 'A bouquet handed to the retiree during the toast is a warm, photo-worthy beat.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_serveware', item: 'Buffet serveware: chafing dishes / warmers, serving spoons, tongs, platters', category: 'rental', qtyFlat: 6, unit: 'pieces', where: ['Party store', 'Rental co', 'Amazon', 'Have/borrow'], unitCostRange: [3, 12], essential: true, buyAt: 'T-3d', note: 'A buffet needs warmers + serving utensils; without them the hot food goes cold and the line jams.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_tableware', item: 'Plates, cutlery, napkins, cups (sturdier disposable for a buffet)', category: 'logistics', qtyPerGuest: 6, unit: 'pieces', where: ['Costco', 'Party store', 'Grocery'], unitCostRange: [0.1, 0.5], essential: true, buyAt: 'T-3d', note: 'COMMONLY UNDER-BOUGHT: buffet + cake + drinks means each guest cycles through several plates, forks, and napkins.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_cleanup', item: 'Trash + recycling bags, dish soap, sponges, bottle bin, leftover containers', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Costco'], unitCostRange: [10, 20], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: a bar + buffet produces a lot of empties and leftovers — stage a recycling bin and to-go containers.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
  ],
  ...
  vendors: [
    ...
    { category: 'Bar / Bartender', required: false, altToDIY: 'A bartender (~$250-400) frees the host to run the party and the speeches; self-serve beer/wine on ice is the free alt', copyByAnswer: { surprise: { 'Full surprise': 'A bartender (~$250-400) frees the host to run the surprise + the speeches; self-serve beer/wine on ice is the free alt', 'Soft surprise (honoree knows, not the details)': 'A bartender (~$250-400) frees the host to run the arrival and the speeches; self-serve beer/wine on ice is the free alt' } }, when: 'T-18d', costRange: [250, 450], costUnit: 'flat' },
    { category: 'Photographer', required: false, altToDIY: 'A 1-2h photographer captures the honoree\'s reaction, group shots, and portraits; a designated phone-photographer friend is the free alt', copyByAnswer: { surprise: { 'Full surprise': 'A 1-2h photographer captures the surprise reaction + group + honoree portraits; a designated phone-photographer friend is the free alt', 'Soft surprise (honoree knows, not the details)': 'A 1-2h photographer captures the honoree\'s arrival reaction + group + honoree portraits; a designated phone-photographer friend is the free alt' } }, when: 'T-14d', costRange: [200, 600], costUnit: 'flat' },
    ...
    { category: 'DJ / Music', required: false, altToDIY: 'A DJ (~$300-600) reads the room, MCs, and cues the tribute\'s music beats; a curated playlist through a real Bluetooth speaker is the free DIY alt', when: 'T-14d', costRange: [300, 600], costUnit: 'flat' },
  ],
