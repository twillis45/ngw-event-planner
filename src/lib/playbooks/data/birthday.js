...
const birthday = {
  ...
  cookLever: { decision: 'food_style', hostCooks: ['Cook/grill yourself'], kind: 'indoor' },
  meta: {
  ...
  decisions: [
    { id: 'theme', label: 'Pick a theme / vibe (or "no theme")', ask: 'Is there a theme, or keep it casual?', options: ['Kids character/theme', 'Milestone (decade) theme', 'Cocktail / grown-up', 'Casual / no theme'], default: 'Casual / no theme', defaultWhy: 'No theme is the default because a theme commits the decor, the cake design and the invites to matching each other, and nothing about a birthday needs that. Change it if the guest of honor is a kid with a favorite character, or if this is a milestone you want the room to say out loud.', when: 'T-21d', blocks: ['decor', 'cake', 'favors'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Theme only steers the look of decor, cake, and invites — cosmetic, cheap, and swappable, so the app can safely default it.', tier: 'reasoned' }, why: 'Theme drives decor, cake design, favors, and invite look. Decide first so the rest is coherent.' },
    { id: 'headcount', ask: 'How many are coming, and how many of them are kids?', label: 'Confirm guest count (and kids vs adults)', options: [], default: null, when: 'T-7d', blocks: ['food', 'cake', 'tableware'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'Every food, cake, and tableware amount scales off the count, so a wrong number mis-buys everything — and only the host knows who is really coming.', tier: 'reasoned' }, why: 'Every food/cake/tableware quantity scales from this. Kids vs adults changes food and drink mix.' },
    { id: 'food_style', ask: 'Cook it yourself, order trays, cater it, or potluck?', label: 'Food style', options: ['Cook/grill yourself', 'Order pizza/trays', 'Drop-off catering', 'Potluck'], default: 'Order pizza/trays', defaultWhy: 'Ordering pizza or trays is the default because it takes the day-of cooking, which is the riskiest part, off the host, and you can change it right up until you place the order. Change it if the party is small enough to cook for comfortably, or if a potluck is what keeps this within budget.', when: 'T-10d', blocks: ['food', 'vendors'], costViaApproach: true, weight: 'med', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Food is the biggest effort-and-cost lever, but trays or drop-off is a safe default and the choice can change right up until you order.', tier: 'reasoned' }, why: 'The biggest effort/cost lever. Trays or drop-off removes the riskiest day-of cooking.' },
    { id: 'alcohol', ask: 'Beer and wine, a full bar, BYOB — or no alcohol?', label: 'Alcohol? (adult parties)', options: ['No alcohol', 'Beer + wine', 'Full bar / signature drink', 'BYOB'], default: 'No alcohol', when: 'T-10d', blocks: ['beverage_purchases'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'Whether to pour at all turns on a kid-vs-adult crowd and who drives home — a host call, though the drinks themselves are cheap to adjust at the store.', tier: 'reasoned' }, why: 'Drives beverage spend, glassware, and whether anyone needs a ride home.' },
    { id: 'dietary', ask: 'Any allergies or diets to cook around?', label: 'Allergies and dietary needs', timingCategory: 'dietary_collection', options: [], default: null, when: 'T-7d', blocks: ['food'], weight: 'high', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'One unflagged nut or dairy allergy is a safety issue, not a courtesy — it must come from the guests before the menu locks.', tier: 'reasoned' }, why: 'Board add: the high-severity allergy risk needs a step that actually asks. Collect from the guest list before locking the menu — one unflagged nut/dairy allergy is a safety issue, not a courtesy.' },
    { id: 'cake', label: 'The cake', ask: 'How are you handling the cake?', options: ['Order a cake', 'Bake it', 'Cupcakes', 'Both cake + treats'], default: 'Order a cake', defaultWhy: 'Ordering the cake is the default because it is the moment everyone sings around, and that is not something to leave riding on your own oven on the morning of the party. Change it if baking it is part of the point for you, or if you are already inside the 3 to 5 days a bakery needs — cupcakes are the answer that late.', when: 'T-7d', dependsOn: ['theme', 'headcount'], weight: 'med', reversibility: 'costly', emotionalWeight: 'high', difmCapable: 'can-derive', deliversHeartMoment: true, priorityBasis: { rationale: 'The lit cake is the moment everyone sings around — the centerpiece of the party, and an ordered one has to be placed ahead so it cannot be left late.', tier: 'reasoned' }, why: 'Ordered cakes need ~3–5 days lead; size scales with headcount (~1 slice/guest).' },
  ],
