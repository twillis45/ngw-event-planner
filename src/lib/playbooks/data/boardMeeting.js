...
const boardMeeting = {
  ...
  decisions: [
    { id: 'format', ask: 'In the room, hybrid, or fully virtual?', label: 'Meeting format', options: ['In-person', 'Hybrid (in-person + remote)', 'Fully virtual'], default: 'Hybrid (in-person + remote)', defaultWhy: 'Hybrid is the default because it is the only format that does not lose a director who cannot travel, and a director lost is quorum at risk — no quorum, no binding votes. It is also the setup most likely to fail live, so it is set early enough to test. Change it if every director is in the room, or none of them are.', when: 'T-30d', blocks: ['av', 'catering', 'access'], weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Every downstream booking — the room, the AV, the catering, the badges — hangs on this call, and hybrid is the setup most likely to fail live, so decide it early enough to test.', tier: 'reasoned' }, why: 'Drives the entire logistics chain — room + AV + catering + parking/badges vs. a video platform + secure remote distribution. Hybrid is the modern default and also the #1 day-of failure point, so it must be decided early enough to test.' },
    { id: 'materials_format', ask: 'A secure portal, numbered printed packets, or both?', label: 'Board materials format', options: ['Secure board portal (digital)', 'Printed packets (numbered, collected)', 'Hybrid: portal + a few printed'], default: 'Secure board portal (digital)', defaultWhy: 'A secure portal is the default because access control and an audit trail come with it, where printed packets have to be numbered, handed out and collected back by a person who can account for every copy. Change it if a director reads on paper, or if the board has no portal to distribute through.', when: 'T-21d', blocks: ['packet', 'confidentiality'], weight: 'high', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'This sets the whole confidentiality workflow for legal governance material — access control, an audit trail, and whether printed copies get numbered and collected.', tier: 'reasoned' }, why: 'Confidential governance material. A secure portal gives access control + audit trail; printed packets must be numbered and collected. This choice sets the whole document-handling and confidentiality workflow.' },
    { id: 'minutes_owner', ask: 'The secretary, counsel, or someone brought in to do it?', label: 'Who takes the minutes', options: ['Board secretary', 'Corporate counsel / paralegal', 'EA / chief of staff', 'Professional minute-taker'], default: 'Board secretary', when: 'T-21d', blocks: ['minutes'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'The minutes are the legal record, so naming who writes them — someone free of voting or facilitation duties — is a real call, and only the org knows who is available.', tier: 'reasoned' }, why: 'Minutes are the legal record and may surface in litigation or books-and-records demands. The note-taker must be designated in advance, free of voting/facilitation duties, and briefed on what to capture (motions, votes, action items) vs. omit.' },
    { id: 'catering_level', ask: 'Coffee and water, breakfast, a working lunch, or nothing?', label: 'Catering level', options: ['Coffee + water only', 'Continental breakfast', 'Working lunch (boxed/buffet)', 'None — directors on their own'], default: 'Coffee + water only', defaultWhy: 'Coffee and water is the default because it fits a session that does not cross a mealtime, and over-catering reads wrong for a governance meeting. Change it to a working lunch if the agenda runs across midday — directors who skip a meal for a long session leave hungry and distracted.', when: 'T-14d', dependsOn: ['format'], blocks: ['catering'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Coffee or a working lunch is the lowest-stakes call here — tie it to the meeting length and keep it functional, never festive.', tier: 'reasoned' }, why: 'Keep it functional, not festive. Tie the level to the meeting length and time of day — coffee for a short morning session, a working lunch only if it runs across midday. Over-catering reads wrong for governance.' },
  ],
  ...
  purchases: [
    ...
    { id: 'p_print', item: 'Printed board packets (numbered) + tabbed binders', category: 'logistics', qtyPerGuest: 1, unit: 'packet', where: ['Print shop', 'Office'], unitCostRange: [5, 15], essential: false, buyAt: 'T-1d', note: 'Only if materials_format includes print. Number each copy for confidentiality and collect them after.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['print-binding-dividers-2026', 'print-booklets-2026'], lastVerified: '2026-08-18', claim: 'Printed and bound board packets per packet. At a published rate card, a spiral-bound 8.5x11in black-and-white book at 100 copies is $6.94 for 50 pages and $20.86 for 300; a 5.5x8.5in is $5.70 at 50 pages. Tabbed dividers add $3.75-7.79 a set. A short numbered packet with tabs lands in this band. NOTE NO PUBLISHER QUOTES BELOW 100 COPIES - every other printer checked is calculator-only - so a board-sized run of ten cannot be priced from published rates and would cost more per packet.', sufficientWhen: 'A quote at the actual board size and page count, against the published 100-copy rate, confirms how far a small run sits above the band.' }, },
    { id: 'p_namecards', item: 'Name cards / table tents + agenda printouts', category: 'logistics', qtyPerGuest: 1, unit: 'set', where: ['Office', 'Print shop'], unitCostRange: [0.5, 2], essential: true, buyAt: 'T-1d', note: 'Helps the minute-taker attribute remarks correctly and keeps roll call clean.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['tablenumbers-holders-2026', 'print-booklets-2026', 'meetingsupplies-quill-2026'], lastVerified: '2026-08-18', claim: 'Name cards and agenda printouts per set. Rigid badge holders are $0.295-0.75 each at 50 a pack and plastic table-number cards $0.08-0.22 each; agenda printouts price with the document tier. A director\'s place set - a card, a holder and a printed agenda - lands in this band.', sufficientWhen: 'A badge-holder 50-pack divided per unit, plus a printed agenda per page, confirm the band.' }, },
    { id: 'p_avsupplies', item: 'AV consumables (HDMI/USB-C adapters, spare batteries, clicker, extension cords)', category: 'logistics', qtyFlat: 1, unit: 'kit', where: ['Office', 'Electronics store', 'Amazon'], unitCostRange: [20, 60], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: the adapter that connects a presenter laptop to the room screen is the classic last-minute scramble.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['av-cables-2026', 'speakers-lights-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced AV components. HDMI cable $9.99-16.99 single (a 2-pack is $8.98, about $4.49 each); USB-C to HDMI adapter $8.59-17.00; outdoor extension cord $13.93 at 16 gauge and $24.99-33.99 at 12 gauge for 25ft; a small Bluetooth speaker from $29.99. Cables and a cord alone sit at this band\'s floor; adding a speaker reaches its ceiling.', sufficientWhen: 'Shelf prices for a cable, an adapter, a cord and a speaker, summed, confirm the band.' }, },
    { id: 'p_notepads', item: 'Notepads, pens, sticky flags for directors', category: 'logistics', qtyPerGuest: 1, unit: 'set', where: ['Office supply'], unitCostRange: [1, 3], essential: false, buyAt: 'T-3d' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['meetingsupplies-quill-2026', 'meetingsupplies-target-2026'], lastVerified: '2026-08-18', claim: 'Desk supplies per attendee set. Notepads are $0.56-1.17 each, pens $0.10-0.19 in bulk (retractables $0.36-1.14), and sticky flags price with the note tier. A set of pad, pen and flags lands in this band; a branded retractable pen pushes it to the ceiling.', sufficientWhen: 'A pad, a pen and a flag pack priced per set confirm the band.' }, },
    { id: 'p_cleanup', item: 'Catering cleanup + secure shredding/disposal supplies', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Office', 'Facilities'], unitCostRange: [5, 15], essential: true, buyAt: 'T-3d', note: 'Confidential printed materials must be shredded, not tossed.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['shredding-service-2026', 'costco-cleaning-2026'], lastVerified: '2026-08-18', claim: 'Cleanup and secure disposal for one meeting. SHREDDING at drop-off is $0.99-1.50 a pound, so a board meeting\'s few pounds of numbered packets is a few dollars; an ON-SITE VISIT is a different purchase entirely at $100-175 and would exceed this band many times over. CLEANUP SUPPLIES: trash bags about 10 cents each, wipes $4.27 a canister. This band prices drop-off shredding plus supplies, not a shredding truck.', sufficientWhen: 'A per-pound drop-off shredding rate applied to the actual packet weight, plus cleanup supplies, confirms the band.' }, },
  ],
