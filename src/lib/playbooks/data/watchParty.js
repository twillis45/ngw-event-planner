...
const watchParty = {
  ...
  // The engine owns the cook question; this says only which answer means THIS
  // host is cooking. The decision and the five generic method tasks come from
  // playbooks/cookLever.js. What stays authored here is genuinely Watch Party's:
  // t_cook's copyByAnswer branch, its run-of-show twin, and r_grill_cold.
  cookLever: { decision: 'menu', hostCooks: ['Wings + chips/dip', 'Chili bar'], kind: 'indoor' },
  meta: {
  ...
  decisions: [
    ...
    { id: 'potluck', ask: 'Are you feeding everybody, or are guests bringing a dish?', label: 'Host-provided or potluck?', options: ['Host provides all', 'Potluck snacks', 'Host feeds, guests bring drinks'], default: 'Host feeds, guests bring drinks', defaultWhy: 'Host feeds, guests bring drinks is the default because handing off the drinks is the easiest way to cut the host load and the bill without anyone coordinating dishes. Change it to a full potluck if you want the food shared out too, or to host-provides-all if you would rather not ask.', when: 'T-7d', blocks: ['food'], costViaApproach: true, weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Host-provides vs potluck is the biggest cost-and-effort lever, but it only reassigns who brings what and defaults to host-feeds-guests-bring-drinks.', tier: 'reasoned' }, why: 'Biggest cost/effort lever — assigning snacks/drinks roughly halves the host load and the bill.' },
    { id: 'alcohol', ask: 'Beer and sodas, BYOB, a full cooler, or keep it dry?', label: 'Drinks', options: ['Beer + soda + water', 'BYOB', 'Full cooler bar', 'Dry / family-friendly'], default: 'Beer + soda + water', when: 'T-5d', blocks: ['beverage_purchases'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'The drink plan sets cooler and ice volume and whether anyone needs a ride home — a host read on the crowd, though cheap to adjust.', tier: 'reasoned' }, why: 'Drives cooler + ice volume over a ~3.5h game and whether anyone needs a ride home.' },
    { id: 'screen', ask: 'One TV, a second screen, a projector — or out to a bar?', label: 'Screen + seating plan', options: ['Living-room TV', 'Add a second screen', 'Projector + screen', 'Bar / out to watch'], default: 'Living-room TV', defaultWhy: 'The living-room TV is the default because it is the screen you already own and the seating is already pointed at it. Change it if more people are coming than the room has sightlines and seats for — that is what makes or breaks a watch party.', when: 'T-5d', blocks: ['rental'], weight: 'high', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'If the game is not on a screen everyone can see, there is no watch party — the one make-or-break call, though the TV setup is easy to arrange.', tier: 'reasoned' }, why: 'Sightlines and enough seats are what make or break a watch party — confirm the stream/channel works and everyone can see the screen before kickoff.' },
  ],
  ...
  purchases: [
    { id: 'p_wings', item: 'Chicken wings', category: 'food', qtyPerGuest: 1, unit: 'lb', where: ['Grocery', 'Costco', 'Butcher'], unitCostRange: [3, 6], essential: true, buyAt: 'T-1d', note: 'Game day runs big — plan ~1 lb per guest, roughly 10–12 split pieces; buy extra if your crowd grazes all afternoon. Wings sell out the day before a big game.', provenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['webstaurant-protein-2026', 'wings-serving-2026'], lastVerified: '2026-09-17', note: 'LIMITATION, and it is the whole story on this line: 1 lb/guest is the figure for wings doing MOST of the food work. This playbook also serves chips and dips, chili, pizza or sliders and dessert, and against that spread every source puts wings at 4-6 split pieces — roughly half this. 1 lb is kept deliberately, because a watch party grazes for three or four hours and running out of wings at half-time is the failure hosts actually report; treat it as generous rather than as the spread-adjusted minimum. Cut it toward 0.5 lb if the rest of the table is heavy.', claim: 'About 1 lb of bone-in wings per guest, or roughly 10-12 split pieces, for an all-afternoon graze. Two independent party-planning publishers and a foodservice portion guide agree that a pound is about 10-12 split pieces (5-6 whole wings at 3.0-3.2oz each), and that 10-12 pieces is the MAIN-EVENT portion while 4-6 is the portion when wings sit beside other mains.', sufficientWhen: 'Settled for the conversion and the two serving tiers: three publishers converge on both. What is NOT settled is which tier a watch party belongs to — the spread here argues for the lower one, and the deliberate choice of the higher one is a hosting judgement stated in the note, not a research finding.' }, alternatives: ['Chicken drumsticks — cheaper per lb than wings, same saucy concept', 'Frozen wings (Costco bag) — cheaper, bake at home'], costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['wings-extension-2026', 'wings-retail-2026', 'chicken-retail-2026'], lastVerified: '2026-08-18', claim: 'Chicken wings 2026, from a land-grant extension market report and a retail guide. Southeast retail average for conventional fresh party wings $2.49/lb, IQF frozen $2.67/lb; the broader retail range is $2.50-5.00/lb, with frozen $2.50-3.50 and fresh or organic $4.50 and above. Wholesale is $1.10/lb, which is why a party-sized bulk buy sits near the band\'s floor while a fresh tray at a supermarket sits at its ceiling.', sufficientWhen: 'One fresh party-wing shelf price and one frozen bulk-bag unit price at the same store confirm the band.' }, },
    ...
    { id: 'p_drinks', item: 'Beer + soda + water', category: 'beverage', qtyPerGuest: 4, unit: 'drinks', where: ['Grocery', 'Costco', 'Liquor store'], unitCostRange: [1, 3], essential: true, buyAt: 'T-3d', note: 'Plan 2 drinks in the first hour then 1 an hour after — the convention caterers buy to. That is 4 a guest for a 3-hour game and 5 for a 4-hour one, so buy the extra round if yours runs long.', provenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['bar-provision-2026', 'drinks-pace-2026'], lastVerified: '2026-09-17', note: 'WHAT THIS NUMBER IS: a purchasing allowance sized so you do not run dry, which is what every source publishing it says it is — not a prediction of how much anyone drinks, and not a target. LIMITATION: the convention is 2 drinks in the first hour then 1 an hour, so 4 covers a THREE-hour game and a four-hour one works out at 5. If your game runs long or goes to overtime, buy the extra round. The caterer publishing the primary figure sells bar service, so the interest runs toward buying more.', claim: 'About 4 drinks per guest across beer, soda and water for a 3-hour game. Four publishers state the same US provisioning convention, and they are NOT independent of the trade — the primary one sells bar service — 2 drinks in the first hour, then 1 per guest per hour — which gives 4 for three hours, 5 for four, 6 for five. The earlier framing of this line as a flat ~1 drink/guest/hour understated the front-loaded first hour that every one of them describes.', sufficientWhen: 'Settled: four independent publishers state the 2-then-1 convention identically, and all stress it is a purchasing convention rather than a consumption estimate. The open question is the watch-party WINDOW — 4 drinks assumes three hours, and an NFL broadcast with pre-game commonly runs four.' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['beer-retail-2026', 'beer-budget-2026', 'soda-12pack-2026', 'bottledwater-case-2026'], lastVerified: '2026-08-16', claim: 'This band is a SUM of separately-priced drink families, not a single quoted item: domestic lager $0.80-1.20 per 12oz (about $20-22 a 24-pack), craft $1.50-3.00; soda $0.25-0.60 a can ($3.00-6.50 a 12-pack); bottled water about $0.17-0.38 a bottle ($4-9 a 24-pack). Each component is cited to its own registered source; the summed band is therefore low-confidence by construction.', sufficientWhen: 'Current shelf prices for one pack of each named component at the same store, summed to the per-serving band, confirm the range.' } },
    { id: 'p_ice', item: 'Ice (coolers + drinks)', category: 'beverage', qtyPerGuest: 1.5, unit: 'lb', where: ['Grocery', 'Gas station'], unitCostRange: [0.2, 0.4], essential: true, buyAt: 'T0', note: 'COMMONLY FORGOTTEN. ~1.5 lb/guest to chill drinks indoors; top up at halftime.', provenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['bar-provision-2026', 'ice-party-2026'], lastVerified: '2026-09-17', note: 'LIMITATION: this line is named for BOTH jobs — coolers and drinks — and the sources treat them separately, at roughly 1 lb a guest for chilling cans and about half a pound per iced drink for serving. One publisher says plainly that a quoted "1 lb per person" usually covers one job, not both. 1.5 lb covers both only because an air-conditioned room melts ice slowly; an indoor game-day figure of 2 lb is equally defensible, and neither publisher adds the 15-20% melt buffer they both recommend. Buy the extra bag if the coolers are doing real work.', claim: 'About 1.5 lb of ice per guest for an indoor watch party. Two independent publishers agree on a 1-3 lb range set by event type and both put an air-conditioned room at its low end, but they do NOT agree on this figure: LoveMyDrink states 1.5 lb for a typical indoor party, while Summer & Cinnamon reads 1 lb for an indoor dinner, 1.5 for a casual party and 2 lb for GAME DAY specifically. 1.5 is the overlap of those two readings, not a number either states for this event.', sufficientWhen: 'Settled for the range and the indoor discount. NOT settled: whether 1.5 lb covers both the cooler and the glass, since the sources price those jobs separately and their sum runs closer to 2 lb for a drinks-heavy afternoon.' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['ice-retail-2026', 'ice-warehouse-2026'], lastVerified: '2026-08-16', claim: 'Bagged ice 2026: warehouse clubs run 10-12c per pound (a 20lb bag is $1.75-2.50 at Sams Club, $1.80-2.50 at Costco); grocery and gas-station bags cluster 23-31c/lb (BJs and 7-Eleven 20lb about $4.49-4.79, Giant 20lb $4.99, Publix 16lb $4.99); small bags and hardware stores reach 41-45c/lb. Convenience ice is more than four times warehouse ice per pound.', sufficientWhen: 'Current shelf prices for one 20lb bag at a warehouse club and one at a grocery store confirm the per-pound spread.' } },
  ...
  schedules: {
    ...
    preparation: [
      ...
      // reading T0-5:00. Same copyByAnswer keys, same back-timing.
      { when: 'T0 -1:30', what: 'Cook the hot food, back-timed to be ready before kickoff',
        copyByAnswer: { cook_method: {
          'Grill or smoker outside': 'Grill is lit and cooking, back-timed to be ready before kickoff',
          'Slow cooker or warming tray': 'Hot dishes are holding — they went on around T0-5:00, not now',
          'Air fryer, in batches': 'Batches running; finished trays holding in a low oven',
          'Store-bought hot or delivered': 'Food collected or delivered and onto serving trays',
        } } },
    ],
    setup: [
    ...
    cleanup: [
      ...
      // 2026-09-23. This playbook's own `note` recorded this row on 2026-09-13
      // as "DISCOVERED, OUT OF SCOPE, NOT FIXED": the bare token 'halftime' is
      // not `during`, not `T0±X`, and not any prose token rosWhenOffset knows,
      // so it returned null and every schedule reader dropped it. The host was
      // never told to swap the trash bag or bag the cans.
      //
      // ANCHORED, NOT NUMBERED. 'after halftime' resolves against this
      // playbook's OWN halftime program beat (T0 +1:45), so the number lives in
      // one place. Writing 'T0 +1:50' here would make this row a second owner of
      // when halftime is, free to drift from the beat it is the cleanup half of.
      //
      // THE GATE COMES FREE, and that is the point. The program beat is gated
      // `not: FORMAT_NO_HALFTIME`, so for UFC, the Masters, Wimbledon, the Draft
      // and an awards show the anchor is not there to find, and this row drops
      // by itself. Restating that gate here would reintroduce precisely the
      // duplication the anchor removes.
      //
      // Reworded formats keep it: the moment index reads the AUTHORED text, so
      // World Series — whose beat renders as "the 7th-inning stretch" — still
      // matches 'halftime' and still gets its cleanup lap.
      { when: 'after halftime', what: 'Restock food, swap trash bag, bag cans for recycling, top up ice' },
