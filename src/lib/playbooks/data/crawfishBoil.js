...
const crawfishBoil = {
  ...
  decisions: [
    { id: 'sacks', noCostEffect: true, weight: 'high', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'The sack count sizes the pot, the propane, and the whole spend, and a live-crawfish order by the sack is real money the host has to commit against real eaters.', tier: 'reasoned' }, label: 'How many sacks of live crawfish?', options: ['1 sack (~30-35 lb)', '2 sacks', '3 sacks', '4+ sacks'], default: '2 sacks', when: 'T-7d', blocks: ['food', 'logistics'], why: 'The whole event sizes off this. Plan ~3-5 lb LIVE crawfish per adult eater (heavy eaters 6-8+). A sack runs ~30-35 lb. 2 sacks (~60-70 lb) feeds roughly a 15-20 person crowd of real eaters. This also sets the pot size and how much propane you burn.' },
    { id: 'cookmethod', weight: 'med', reversibility: 'costly', emotionalWeight: 'low', difmCapable: 'needs-host', priorityBasis: { rationale: 'This sets whether the host runs a propane rig for hours or pays for a boiled-by-the-pound pickup — a real cost-and-effort tradeoff to weigh.', tier: 'reasoned' }, label: 'Boil it yourself or order it cooked?', options: ['Boil it yourself (propane pot)', 'Order live + boil yourself', 'Order it boiled by the pound (pickup)'], default: 'Boil it yourself (propane pot)', when: 'T-10d', blocks: ['food', 'logistics'], costFactors: { 'Order it boiled by the pound (pickup)': 1.6 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Heuristic: ordering cooked crawfish adds ~60% markup. Needs price comparison.', claim: 'Ordering crawfish already boiled by the pound adds ~60% cost over buying and boiling live', sufficientWhen: 'Price comparison of ≥2 live-sack vs boiled-by-pound quotes from the same Gulf Coast or regional seafood supplier' }, affects: ['p_crawfish'], why: 'Boiling yourself is the tradition and the fun, but it needs the pot, the burner, the propane, and someone running it for hours. Ordering it already boiled by the pound from a seafood place is a real, common shortcut when you don\'t have the rig or the time.' },
    { id: 'potsize', standsDownWhen: { id: 'cookmethod', in: ['Order it boiled by the pound (pickup)'] }, weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Pot and burner size follow straight from the sack count — the app can size it at about one sack per pot, and the rig is swappable before boil day.', tier: 'reasoned' }, label: 'Pot + burner size', options: ['60-80 qt pot + high-BTU jet burner (1-2 sacks)', '100+ qt pot + jet burner (3+ sacks)', 'Two pots running in shifts'], default: '60-80 qt pot + high-BTU jet burner (1-2 sacks)', when: 'T-7d', blocks: ['logistics'], why: 'You boil ~one sack per pot per batch. A 60-80 qt pot handles a sack at a time; bigger crowds run a 100+ qt pot or two pots in shifts. The burner must be a high-BTU outdoor jet burner — a backyard grill burner won\'t get a full pot to a rolling boil.' },
    { id: 'heat', weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Heat level is personal taste and a mild-plus-hot split is easy to run — a cheap seasoning call the app can default to medium.', tier: 'reasoned' }, label: 'Seasoning heat level', options: ['Mild (family / kids)', 'Medium (classic)', 'Hot (Louisiana hot)', 'Two batches — mild + hot'], default: 'Medium (classic)', when: 'T-7d', blocks: ['food'], costFactors: { 'Mild (family / kids)': 0.9, 'Hot (Louisiana hot)': 1.15, 'Two batches — mild + hot': 1.3 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Heuristic: mild saves 10% on spices; hot adds 15%; two batches doubles seasoning cost. Needs spice pricing verification.', claim: 'Mild seasoning saves ~10% on spice cost; hot adds ~15%; running two separate batches roughly doubles seasoning cost', sufficientWhen: 'Pricing comparison of Zatarain\'s/Louisiana Fish Fry plus cayenne quantities across mild, medium, and hot ratios confirms the stated deltas' }, affects: ['p_season'], why: 'The seasoning (Zatarain\'s or Louisiana Fish Fry crab/crawfish boil + cayenne + lemon + salt) carries the whole dish, and heat is personal. A mild-and-hot split keeps kids and tender mouths happy without dialing the whole pot down.' },
    { id: 'sides', weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'The fixings soak up as much seasoning as the crawfish, but they are cheap add-ins decided late — potatoes, corn, and sausage are a safe default.', tier: 'reasoned' }, label: 'What goes in the pot with the crawfish?', options: ['Red potatoes + corn + smoked sausage', 'Add onions, garlic, mushrooms', 'Add the works (lemons, artichoke, Brussels)'], default: 'Add onions, garlic, mushrooms', when: 'T-7d', dependsOn: ['cookmethod'], blocks: ['food'], costFactors: { 'Red potatoes + corn + smoked sausage': 0.55, 'Add the works (lemons, artichoke, Brussels)': 1.4 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Heuristic: classic sides are 55% of base; full spread adds 40% uplift from premium vegetables. Needs produce pricing verification.', claim: 'Red potatoes, corn, and smoked sausage alone cost ~55% of the full sides budget; adding artichokes and Brussels sprouts adds ~40% uplift', sufficientWhen: 'Grocery price comparison of classic vs premium add-on vegetables at typical per-sack boil quantities confirms the stated deltas' }, affects: ['p_aromatics'], why: 'The classics — small red potatoes, corn on the cob, smoked sausage or andouille — boil right alongside. Onions, whole garlic, mushrooms, and halved lemons go in too. The fixings soak up as much seasoning as the crawfish, so most cooks add more than they think.' },
    { id: 'drinks', weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Cold beer with plenty of water and ice is the boil default — a cheap store call that is easy to adjust.', tier: 'reasoned' }, label: 'Drinks', options: ['Cold beer + soda + water', 'Add a non-alcoholic / kids spread', 'Beer + soda + water + ice tea'], default: 'Cold beer + soda + water', when: 'T-7d', blocks: ['beverage'], costFactors: { 'Add a non-alcoholic / kids spread': 1.15, 'Beer + soda + water + ice tea': 1.05 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Heuristic: premium non-alcoholic options add 15%; iced tea adds 5% to baseline. Needs beverage cost verification.', claim: 'Adding a premium non-alcoholic spread (juices, lemonade) adds ~15% to beverage cost; adding iced tea alone adds ~5%', sufficientWhen: 'Itemized beverage cost comparison of base (beer, soda, water) vs each add-on tier at typical per-guest quantities' }, affects: ['p_drinks'], why: 'Cold beer is the crawfish-boil drink, with soda, water, and plenty of ice for everybody. It\'s a long afternoon outside in spring — over-buy ice and water, not just beer.' },
  ],
  ...
  purchases: [
    { id: 'p_crawfish', item: 'Live crawfish (by the sack, ~30-35 lb/sack)', category: 'food', qtyPerGuest: 4, unit: 'lb', where: ['Seafood market', 'Crawfish supplier / dock', 'Grocery seafood counter (in season)'], unitCostRange: [3, 7], essential: true, buyAt: 'T-1d', note: 'The whole point. Plan ~3-5 lb LIVE per adult eater (heavy eaters 6-8+). A sack is ~30-35 lb. Buy in season (spring) and as close to boil day as possible — they must be ALIVE going into the pot.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: '~3-5 lb live crawfish/adult eater and ~30-35 lb/sack are common Louisiana boil planning norms.', claim: 'Adult eaters consume ~3-5 lb of live crawfish per person; a standard sack weighs ~30-35 lb', sufficientWhen: 'Verified against Louisiana crawfish supplier guidance or LSU AgCenter extension-service planning materials' }, alternatives: ['Shrimp — far cheaper per lb, same boil seasoning treatment', 'Frozen crawfish tails — if live unavailable, no shell-peeling required'] },
    { id: 'p_season', item: 'Crawfish/crab boil seasoning (Zatarain\'s / Louisiana Fish Fry) + cayenne + salt', category: 'food', qtyPerGuest: 0.2, unit: 'lb', where: ['Grocery', 'Seafood market'], unitCostRange: [1.5, 4], essential: true, buyAt: 'T-3d', note: 'The flavor. Powdered or liquid concentrate (or both) + extra cayenne to taste + a cup or so of salt per pot. Plan ~1 lb of boil seasoning per sack as a starting point; the soak does the rest.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: '~1 lb boil seasoning/sack is a common starting ratio; cooks adjust to taste.', claim: 'A 30-35 lb sack of crawfish requires approximately 1 lb of boil seasoning as a starting ratio', sufficientWhen: 'Verified against Zatarain\'s or Louisiana Fish Fry package guidance and ≥2 regional boil recipes confirming the per-sack ratio' } },
    { id: 'p_lemon', item: 'Lemons (halved into the pot)', category: 'food', qtyPerGuest: 0.25, unit: 'lemon', where: ['Grocery'], unitCostRange: [0.4, 0.8], essential: true, buyAt: 'T-1d', note: 'Halved and squeezed into the boil — brightens the seasoning. 4-6 per pot is typical.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_potatoes', item: 'Small red potatoes', category: 'food', qtyPerGuest: 0.4, unit: 'lb', where: ['Grocery'], unitCostRange: [0.8, 1.5], essential: true, buyAt: 'T-1d', note: 'Go in first (longest cook). They drink up seasoning — a boil favorite. ~2 small potatoes/person.', alternatives: ['Yukon gold potatoes — similar texture, usually same price', 'Russet potatoes cut in chunks — cheapest option, absorbs seasoning well'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['bls-produce-2026', 'usda-produce-outlook-2026'], lastVerified: '2026-08-16', claim: 'BLS average retail price for potatoes was $0.94/lb in July 2026 (series APU0000712112, released 2026-08-12), down 3.7% on the year, with USDA ERS projecting fresh vegetables up 6.8% across 2026. NOTE THE VARIETY GAP: the BLS series tracks WHITE potatoes and this line buys small red or new potatoes, which typically carry a modest premium over the white-potato average — so this is the right commodity at the right unit, one variety across.', sufficientWhen: 'A current per-pound shelf price for small red or new potatoes at one grocery store confirms the premium over the BLS white-potato average.' } },
    { id: 'p_corn', item: 'Corn on the cob (halved ears)', category: 'food', qtyPerGuest: 1, unit: 'half-ear', where: ['Grocery'], unitCostRange: [0.3, 0.7], essential: true, buyAt: 'T-1d', note: 'Halved ears go in near the end. Frozen also helps cool the pot before the soak. ~1 half-ear/person and then some.', alternatives: ['Frozen corn on the cob — works fine in the boil, cheaper out of season'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_sausage', item: 'Smoked sausage / andouille', category: 'food', qtyPerGuest: 0.25, unit: 'lb', where: ['Grocery', 'Butcher'], unitCostRange: [3, 6], essential: true, buyAt: 'T-1d', note: 'Cut into chunks; boils alongside and soaks up the seasoning. Andouille is the Louisiana classic.', alternatives: ['Kielbasa — widely available, cheaper than andouille', 'Turkey sausage — budget swap, still smoky'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_aromatics', item: 'Onions, whole garlic heads, mushrooms', category: 'food', qtyPerGuest: 0.3, unit: 'lb', where: ['Grocery'], unitCostRange: [0.6, 1.4], essential: false, buyAt: 'T-1d', note: 'Quartered onions, halved garlic heads, and whole mushrooms in the pot. The mushrooms become a seasoned-soak favorite.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_propane', item: 'Propane (full 20 lb tank + backup)', category: 'logistics', qtyFlat: 1, unit: 'tank', where: ['Hardware store', 'Gas station', 'Propane exchange'], unitCostRange: [18, 30], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: a high-BTU jet burner drinks propane bringing a full pot to a rolling boil. Start FULL, keep a backup — a dead tank mid-boil ruins the timing.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_paper', item: 'Newspaper / kraft butcher paper (table cover)', category: 'logistics', qtyFlat: 1, unit: 'roll', where: ['Grocery', 'Hardware store', 'Restaurant supply'], unitCostRange: [6, 15], essential: true, buyAt: 'T-3d', note: 'The boil gets DUMPED on a paper-covered table — no plates. A roll of kraft/butcher paper or stacks of newspaper. Cleanup is just rolling it up with the shells.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_drinks', item: 'Cold beer, soda, water', category: 'beverage', qtyPerGuest: 4, unit: 'drinks', where: ['Grocery', 'Costco/Sam\'s'], unitCostRange: [0.7, 2], essential: true, buyAt: 'T-3d', note: 'Cold beer is the boil drink; soda + water for everyone. It\'s a long, hot afternoon — over-buy water.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: '~4 drinks/guest over a 5h spring afternoon outdoors.', claim: 'Guests consume ~4 drinks each over a 5-hour outdoor spring crawfish boil', sufficientWhen: 'Verified against outdoor event beverage consumption norms for a 5h warm-weather afternoon gathering' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['beer-retail-2026', 'beer-budget-2026', 'soda-12pack-2026', 'bottledwater-case-2026'], lastVerified: '2026-08-16', claim: 'This band is a SUM of separately-priced drink families, not a single quoted item: domestic lager $0.80-1.20 per 12oz (about $20-22 a 24-pack), craft $1.50-3.00; soda $0.25-0.60 a can ($3.00-6.50 a 12-pack); bottled water about $0.17-0.38 a bottle ($4-9 a 24-pack). Each component is cited to its own registered source; the summed band is therefore low-confidence by construction.', sufficientWhen: 'Current shelf prices for one pack of each named component at the same store, summed to the per-serving band, confirm the range.' } },
    { id: 'p_ice', item: 'Ice (drinks + coolers)', category: 'beverage', qtyPerGuest: 2.5, unit: 'lb', where: ['Grocery', 'Gas station', 'Ice house'], unitCostRange: [0.2, 0.4], essential: true, buyAt: 'T0', note: 'COMMONLY UNDER-BOUGHT. A spring boil burns through ice — ~2.5 lb/guest for cold drinks (more on a hot day).' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['ice-retail-2026', 'ice-warehouse-2026'], lastVerified: '2026-08-16', claim: 'Bagged ice 2026: warehouse clubs run 10-12c per pound (a 20lb bag is $1.75-2.50 at Sams Club, $1.80-2.50 at Costco); grocery and gas-station bags cluster 23-31c/lb (BJs and 7-Eleven 20lb about $4.49-4.79, Giant 20lb $4.99, Publix 16lb $4.99); small bags and hardware stores reach 41-45c/lb. Convenience ice is more than four times warehouse ice per pound.', sufficientWhen: 'Current shelf prices for one 20lb bag at a warehouse club and one at a grocery store confirm the per-pound spread.' } },
    { id: 'p_napkins', item: 'Paper towels, napkins, trash bags, gloves', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Costco/Sam\'s'], unitCostRange: [12, 25], essential: true, buyAt: 'T-3d', note: 'Crawfish is a hands-and-shells mess. Stacks of paper towels, plenty of napkins, heavy trash bags, and a roll of gloves for peeling-shy guests.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_cups', item: 'Disposable cups, bowls (for shells / fixings)', category: 'rental', qtyPerGuest: 1.5, unit: 'set', where: ['Grocery', 'Party store'], unitCostRange: [0.2, 0.6], essential: false, buyAt: 'T-3d', note: 'No plates needed for the dump, but cups for drinks and a few bowls for shells/fixings help.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
  ],
  ...
  risks: [
    ...
    { id: 'r_fuel', trigger: 'Propane runs out / jet burner won\'t light mid-boil', severity: 'high', mitigation: 'Test-fire the burner at a week out; start with a FULL tank and keep a backup on hand — a high-BTU burner empties a tank fast.' },
    ...
    { id: 'r_supply', trigger: 'Sacks sell out or price/size swings with the catch', severity: 'med', mitigation: 'Reserve sacks early (10 days out) in spring while they\'re running; confirm at T-3d; have a boiled-by-the-pound fallback place in your pocket.' },
  ...
  schedules: {
    ...
    program: [
      // Same ownership fix as lowCountryBoil: papering and stripping are the two
      // jobs everybody assumes somebody else took, and the strip is the one
      // nobody volunteers for once the eating is done.
      { when: 'T0 +5m', what: 'Doors: table papered and taped down by two people before anyone arrives, drinks on ice, burner lit and attended' },
      ...
      { when: 'T0 +4h', what: 'Wind down: shells bagged, burner off and cooled, table stripped by the same pair who papered it' },
    ],
    cleanup: [
