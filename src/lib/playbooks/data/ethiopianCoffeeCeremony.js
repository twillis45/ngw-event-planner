...
const ethiopianCoffeeCeremony = {
  ...
  decisions: [
    { id: 'scope', label: 'Ceremony only, or ceremony + full meal?', options: ['Ceremony + snacks only', 'Ceremony + full injera-and-wat meal'], default: 'Ceremony + full injera-and-wat meal', when: 'T-7d', blocks: ['food', 'beverage_purchases'], costFactors: { 'Ceremony + snacks only': 0.45 }, affects: ['p_injera', 'p_meatwat', 'p_veganwat', 'p_spices'], why: 'The biggest effort + cost lever. Ceremony-only (coffee + popcorn + bread) is gracious on its own; adding the wat spread roughly doubles the cook.' },
    { id: 'injera_source', label: 'Make injera or buy it?', options: ['Buy from an Ethiopian market/restaurant', 'Make from teff at home'], default: 'Buy from an Ethiopian market/restaurant', when: 'T-7d', blocks: ['food'], costFactors: { 'Make from teff at home': 0.7 }, affects: ['p_injera'], why: 'Buying injera is normal and smart — teff sourdough ferments for days and is hard to get right. DC-area markets and restaurants sell it fresh; let them carry it.' },
    { id: 'fasting_spread', label: 'Vegan / Orthodox-fasting spread?', options: ['Always include a full vegan (ye-tsom) spread', 'Vegan dishes only (fasting day)', 'Meat + vegan both'], default: 'Always include a full vegan (ye-tsom) spread', when: 'T-7d', blocks: ['food'], costFactors: { 'Vegan dishes only (fasting day)': 0.15 }, affects: ['p_meatwat'], why: 'Vegan fasting food (shiro, misir, gomen) is central, not a side accommodation — Orthodox guests may be fasting (Wed/Fri, Lent). Always offer it; check whether the date is a fasting day.' },
