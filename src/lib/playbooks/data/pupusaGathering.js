...
const pupusaGathering = {
  ...
  decisions: [
    { id: 'make_vs_order', label: 'Make from scratch, order, or hybrid?', options: ['Make all from scratch (comal day)', 'Order from a pupusería', 'Hybrid — host makes some, orders the rest'], default: 'Hybrid — host makes some, orders the rest', when: 'T-7d', blocks: ['food', 'griddle'], weight: 'high', reversibility: 'reversible', emotionalWeight: 'med', difmCapable: 'can-derive', priorityBasis: { rationale: 'Scratch-versus-order is the single biggest effort lever and drives every masa and filling quantity, yet it stays changeable up until you reserve a pupusería — and the hybrid default keeps the communal shaping while backstopping volume.', tier: 'reasoned' }, costFactors: { 'Make all from scratch (comal day)': 0.75, 'Order from a pupusería': 1.3 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Cost factor heuristics need verification against actual pricing.', claim: 'Making all pupusas from scratch saves ~25% vs the hybrid baseline; ordering all from a pupusería adds ~30%', sufficientWhen: '≥2 pupusería price quotes and an ingredient cost tally confirm the scratch-vs-order cost delta within 15 percentage points' }, affects: ['p_masa', 'p_quesillo', 'p_chicharron', 'p_beans'], why: 'The single biggest effort lever. Scratch is the heart of it but slow on one comal; hybrid lets the shaping stay communal while a pupusería backstops volume so nobody is stuck at the griddle all night.' },
    { id: 'fillings', label: 'Fillings spread', options: ['Revueltas only (chicharrón + cheese + beans)', 'Revueltas + queso + frijol con queso', 'Full spread incl. loroco + ayote/squash', 'Veg-friendly (queso, frijol, loroco, ayote)'], default: 'Revueltas + queso + frijol con queso', when: 'T-7d', dependsOn: ['make_vs_order'], blocks: ['food'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'The filling mix sets the masa, cheese, bean, and pork quantities but swaps freely, and the safe default already keeps a plain queso and a bean option so kids and non-pork eaters are covered.', tier: 'reasoned' }, costFactors: { 'Revueltas only (chicharrón + cheese + beans)': 0.85, 'Full spread incl. loroco + ayote/squash': 1.2, 'Veg-friendly (queso, frijol, loroco, ayote)': 0.9 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Cost factor heuristics need verification against actual pricing.', claim: 'Revueltas-only saves ~15%; a full loroco/ayote spread adds ~20%; a veg-friendly spread saves ~10% vs the standard baseline', sufficientWhen: 'ingredient cost quotes for quesillo, loroco, chicharrón, and ayote at a DMV Latin market confirm the relative cost deltas within 10 percentage points' }, affects: ['p_quesillo', 'p_chicharron', 'p_beans', 'p_loroco'], why: 'Drives masa, quesillo, beans, and chicharrón quantities. Revueltas is the classic; always keep a plain queso and a bean option so kids and non-pork eaters are covered.' },
    { id: 'curtido_ahead', standsDownWhen: { id: 'make_vs_order', in: ['Order from a pupusería'] }, label: 'Curtido — made ahead (it needs days)', options: ['Make 3+ days ahead (properly fermented)', 'Make 1–2 days ahead (lightly soured)', 'Buy jarred curtido'], default: 'Make 3+ days ahead (properly fermented)', when: 'T-4d', blocks: ['food'], weight: 'med', reversibility: 'costly', emotionalWeight: 'med', difmCapable: 'can-derive', priorityBasis: { rationale: 'Curtido has to sour ahead of time, so the timing sets a hard task well before the gathering and a missed window cannot be fixed on the day — and a pupusa table without properly soured curtido feels incomplete.', tier: 'reasoned' }, costFactors: { 'Buy jarred curtido': 1.2 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Cost factor heuristics need verification against actual pricing.', claim: 'Buying jarred curtido costs ~20% more than making it from scratch', sufficientWhen: 'curtido ingredient cost vs jarred product price at a Latin market confirms the ~20% premium' }, affects: ['p_curtido_veg'], why: 'Curtido is not a same-day side — it needs to sour. Decide early because the made-ahead option sets a hard task days before the gathering. No curtido = it is not really a pupusa table.' },
    { id: 'drinks', label: 'Drinks', options: ['Horchata + tamarindo (Salvadoran, morro seed)', 'Aguas frescas + sodas only', 'Add a grown section (Pilsener / Suprema / beer)', 'Coffee + sweet bread to close'], default: 'Horchata + tamarindo (Salvadoran, morro seed)', when: 'T-5d', blocks: ['beverage_purchases'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Drinks are a cheap, late, swappable choice and the Salvadoran horchata-and-tamarindo default is always a safe pairing with the masa — the lowest-stakes call on the board.', tier: 'reasoned' }, why: 'Salvadoran horchata is morro-seed based — different from Mexican rice horchata — and pairs with the masa. A grown section (Pilsener, Suprema) is optional; aguas frescas keep it all-ages.' },
  ],
  ...
  purchases: [
    { id: 'p_masa', item: 'Masa (instant corn masa harina or fresh masa)', category: 'food', qtyPerGuest: 0.7, unit: 'lb', where: ['Latin market', 'Grocery', 'Tortillería'], unitCostRange: [1, 3], essential: true, buyAt: 'T-3d', note: '~3–4 pupusas/adult; ~0.15–0.2 lb masa each. Fresh masa from a tortillería shapes best if you can get it.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: '3–4 pupusas/adult × ~0.15–0.2 lb masa/pupusa.', claim: 'Each adult eats 3-4 pupusas; each pupusa requires ~0.15-0.2 lb of masa, for ~0.7 lb masa per adult total', sufficientWhen: '≥2 pupusería or home-cook sources confirm the 3-4 pupusas/adult and ~0.15-0.2 lb masa/pupusa estimates' }, alternatives: ['Maseca instant masa (bag) — widely available, cheaper than fresh', 'Bob\'s Red Mill masa harina — if Maseca sold out, same result'], costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['masa-retail-2026', 'bls-staples-2026'], lastVerified: '2026-08-18', claim: 'Instant corn masa flour priced per pound. Maseca is $4.29 for a 4lb bag at Target and Kroger, a stated $0.07 an ounce - about $1.07/lb, which is the floor of this band. Fresh masa from a tortilleria prices above bagged instant, which is what the ceiling covers. A Walmart figure was excluded because the site returned a CAPTCHA rather than a page.', sufficientWhen: 'A 4lb bagged masa price and a fresh-masa price from a tortilleria confirm both ends of the band.' }, },
    { id: 'p_quesillo', item: 'Quesillo (Salvadoran melting cheese) or mozzarella blend', category: 'food', qtyPerGuest: 0.25, unit: 'lb', where: ['Latin market', 'Grocery'], unitCostRange: [4, 7], essential: true, buyAt: 'T-3d', note: 'Quesillo is the soul of queso + revueltas pupusas. Mozzarella/Oaxaca is the common DMV substitute.', alternatives: ['Whole-milk mozzarella — widely available substitute, melts similarly', 'Oaxacan cheese — closer to quesillo, often found at Latin markets'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['cheese-sliced-2026', 'cheese-deli-2026', 'charcuterie-grocery-2026'], lastVerified: '2026-08-18', claim: 'A melting cheese, priced in the everyday tier rather than the specialty one. Packaged block cheddar is $3.58-3.78/lb at Walmart and processed slices $4.86/lb; a deli counter cuts American to order at $5.99/lb. Quesillo and mozzarella price with these rather than with SPECIALTY cheese at about $15/lb, which is a cheese-board product - using that figure here would roughly treble the number. Quesillo from a Latin grocery may carry a small premium over a mass-market block, which is where the band\'s ceiling sits.', sufficientWhen: 'A per-pound price for quesillo at a Latin grocery alongside a mass-market mozzarella block confirms both ends of the band.' }, },
    { id: 'p_chicharron', item: 'Chicharrón (ground pork) for revueltas', category: 'food', qtyPerGuest: 0.15, unit: 'lb', where: ['Latin market', 'Butcher'], unitCostRange: [4, 8], essential: false, buyAt: 'T-3d', note: 'Ground/cooked pork for revueltas. Skip for the veg-friendly spread.', alternatives: ['Ground pork + salt — DIY chicharron filling, much cheaper', 'Skip it — frijol con queso is the vegetarian option that everyone can eat'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['costco-pork-2026', 'usda-meat-2026'], lastVerified: '2026-08-18', claim: 'Ground pork for chicharron, priced per pound. Costco pork and the USDA meat series put pork chops near $4.33/lb, with ground and shoulder cuts around and below that; a Latin butcher grinding to order prices above a supermarket tray, which is where this band\'s ceiling comes from. Beef for comparison averages $9.64/lb all-fresh, well above this row.', sufficientWhen: 'A per-pound ground pork price at a supermarket and at a Latin butcher confirm the band.' }, },
    { id: 'p_beans', item: 'Refried beans (frijoles molidos) for frijol con queso', category: 'food', qtyPerGuest: 0.2, unit: 'lb', where: ['Latin market', 'Grocery'], unitCostRange: [1.5, 3], essential: true, buyAt: 'T-3d', note: 'Frijol con queso is the everyone-can-eat-it pupusa — always have it.', alternatives: ['Canned refried beans — if homemade unavailable, dress with cumin and lard', 'Dried pinto beans (cook from scratch) — cheapest option, takes 2 hours'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['bls-staples-2026', 'usda-produce-outlook-2026'], lastVerified: '2026-08-18', claim: 'Beans priced per pound. BLS puts dried beans, any type, at $1.704/lb (February 2026); organic heirloom is a separate premium tier at $4.00-6.50/lb and is excluded. Refried or baked beans add aromatics and a little fat to that base, which is why the band sits at and slightly above the raw commodity rather than on it.', sufficientWhen: 'One bag price for dried beans alongside a can of prepared beans confirms both ends of the band.' }, },
    { id: 'p_loroco', item: 'Loroco (and/or ayote/squash) for the spread', category: 'food', qtyFlat: 1, unit: 'jar/bunch', where: ['Latin market'], unitCostRange: [4, 9], essential: false, buyAt: 'T-3d', note: 'Loroco con queso is a signature Salvadoran filling — sold frozen/jarred at Latin markets. Ayote/squash is the milder cousin.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['loroco-importer-2026', 'loroco-mexgrocer-2026'], lastVerified: '2026-08-18', claim: 'Jarred loroco in brine, 32oz. A Central American importer lists it at $6.39 and a Latin specialty grocer lists Goya at $13.95 for the SAME 32oz format - an importer-versus-retail spread of more than 2x, not a figure to average. The importer price sits at this band\'s floor and the Goya listing above its ceiling, so the band prices the importer and mid-market end.', sufficientWhen: 'Two current 32oz jar prices, one importer and one grocery, confirm where in that spread a host actually buys.' }, },
    { id: 'p_curtido_veg', item: 'Curtido ingredients (cabbage, carrots, onion, oregano, vinegar)', category: 'food', qtyFlat: 1, unit: 'batch', where: ['Grocery', 'Latin market'], unitCostRange: [6, 12], essential: true, buyAt: 'T-3d', note: 'NON-NEGOTIABLE. Buy at 3 days out so you can jar it at 4 days out–3d and let it sour. A pupusa table without curtido is incomplete.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['usda-feature-produce-2026', 'bls-saladveg-2026'], lastVerified: '2026-08-18', claim: 'A fermented slaw batch. Green cabbage is $0.70/lb advertised nationally and $0.81/lb at one shelf; carrots and onion price with the produce outlook, which has fresh vegetables rising 6.8% across 2026. A batch for a party takes several pounds of cabbage plus aromatics and vinegar, which brackets this band.', sufficientWhen: 'A per-pound cabbage price times the batch quantity, plus aromatics, confirms the band.' }, },
    { id: 'p_salsa', item: 'Salsa roja ingredients (tomatoes, onion, garlic, peppers)', category: 'food', qtyFlat: 1, unit: 'batch', where: ['Grocery', 'Latin market'], unitCostRange: [5, 10], essential: true, buyAt: 'T-1d', note: 'NON-NEGOTIABLE companion to curtido. A thin cooked tomato sauce, ladled over.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['bls-saladveg-2026', 'usda-produce-outlook-2026'], lastVerified: '2026-08-18', claim: 'Salsa roja ingredients for one batch. BLS field-grown tomatoes are $2.154/lb and are the bulk of the cost; onions, garlic and dried peppers price with the produce outlook, which has fresh vegetables rising 6.8% across 2026. A batch for a party takes several pounds of tomato plus aromatics, which brackets this band. NOTE tomatoes were 19.5% higher year on year to June 2026, so this band dates faster than most.', sufficientWhen: 'A per-pound tomato price times the batch quantity, plus aromatics, confirms the band.' }, },
    { id: 'p_oil', item: 'Cooking oil + water bowl for shaping', category: 'logistics', qtyFlat: 1, unit: 'supply', where: ['Grocery'], unitCostRange: [4, 8], essential: true, buyAt: 'T-3d', note: 'Oiled/wet hands keep the masa from sticking; a light film on the comal. Commonly forgotten until you are mid-shape.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_horchata', item: 'Horchata (morro seed) + tamarindo mix', category: 'beverage', qtyPerGuest: 2, unit: 'drinks', where: ['Latin market', 'Grocery'], unitCostRange: [0.75, 2], essential: true, buyAt: 'T-3d', note: 'Salvadoran horchata is morro-seed based — NOT Mexican rice horchata. Tamarindo alongside.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_beer', item: 'Beer (Pilsener / Suprema) or sodas — grown/all-ages section', category: 'beverage', qtyPerGuest: 2, unit: 'drinks', where: ['Latin market', 'Liquor store', 'Grocery'], unitCostRange: [1, 3], essential: false, buyAt: 'T-3d', note: 'Salvadoran Pilsener/Suprema if you can find them; sodas (Kolashampan) keep it all-ages.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['beer-retail-2026', 'beer-budget-2026', 'soda-12pack-2026'], lastVerified: '2026-08-16', claim: 'This band is a SUM of separately-priced drink families, not a single quoted item: domestic lager $0.80-1.20 per 12oz (about $20-22 a 24-pack), craft $1.50-3.00; soda $0.25-0.60 a can ($3.00-6.50 a 12-pack). Each component is cited to its own registered source; the summed band is therefore low-confidence by construction.', sufficientWhen: 'Current shelf prices for one pack of each named component at the same store, summed to the per-serving band, confirm the range.' } },
    { id: 'p_tableware', item: 'Plates, napkins, forks (sturdy — pupusas are hot + greasy)', category: 'rental', qtyPerGuest: 2, unit: 'set', where: ['Grocery', 'Costco', 'Party store'], unitCostRange: [0.2, 0.6], essential: true, buyAt: 'T-3d' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['disposables-bulk-2026', 'disposables-partyqty-2026'], lastVerified: '2026-08-16', claim: 'Disposable tableware runs about 6 cents a plate in bulk packs (roughly $18 per 300) and $0.25-0.40 a plate at a grocery store, with bulk restaurant supply at $0.08-0.15; cups, cutlery and napkins bought in bulk alongside them save $100-200 across a party.', sufficientWhen: 'Current shelf prices for one bulk plate pack and the same item at a grocery store confirm the per-piece spread.' } },
    { id: 'p_cleanup', item: 'Trash bags, paper towels, foil + containers for leftovers', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery'], unitCostRange: [7, 18], essential: true, buyAt: 'T-3d', note: 'Foil/containers to send pupusas home — guests will ask. Paper towels for the greasy comal.' , costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['costco-cleaning-2026', 'trashbags-retail-2026'], lastVerified: '2026-08-15', claim: 'A cleanup kit runs $7-18 as the SUM of its parts: about a dozen trash and recycling bags at 10 cents each from a warehouse or 11-15 cents at grocery, two rolls of paper towels at about $1.97 warehouse, and a canister of wipes at about $4.27 or a dish-soap pack at $14.74 shared across events.', sufficientWhen: 'CONFIDENCE IS LOW ON PURPOSE: no source prices a cleanup kit, because nobody sells one. This band is a sum of individually-priced components, so treat it as an envelope rather than a quote. The spread is the CHANNEL - warehouse packs against a grocery shelf - and a host who already owns soap and towels lands well under the floor. Kits that also carry gloves, foil or to-go containers are a different bundle.' } },
  ],
  ...
  risks: [
    { id: 'r_curtido_time', trigger: 'Curtido made too late — not soured', severity: 'med', mitigation: 'Make it at 4 days out–3d so it can ferment; buy a backup jar of curtido as insurance.' },
    ...
    { id: 'r_supply', trigger: 'Quesillo / loroco / fresh masa sold out', severity: 'med', mitigation: 'Source at 5 days out from a Latin market; mozzarella subs for quesillo; instant masa harina subs for fresh masa.' },
