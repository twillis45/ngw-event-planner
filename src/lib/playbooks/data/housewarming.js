...
const housewarming = {
  ...
  purchases: [
    { id: 'p_cheese', item: 'Cheese (3-4 varieties for the board)', category: 'food', qtyPerGuest: 0.15, unit: 'lb', where: ['Grocery', 'Cheese shop', 'Costco'], unitCostRange: [6, 14], essential: true, buyAt: 'T-1d', note: 'Anchor of the grazing board. ~2-3 oz/guest across the spread.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'Grazing-board portion rule of thumb; volume sized to peak overlap, not the full open-house list.', claim: '~2–3 oz (~0.15 lb) of cheese per guest on a grazing board is sufficient when sized to peak overlap, not the full open-house list', sufficientWhen: 'A catering or hosting guide confirms ~2–3 oz cheese/guest as the standard grazing-board portion for a drop-in party' }, alternatives: ['One good cheese + store-brand crackers — halves the cost', 'Pre-made cheese plate (Trader Joe\'s) — convenient budget option'] },
    { id: 'p_charc', item: 'Charcuterie / cured meats', category: 'food', qtyPerGuest: 0.1, unit: 'lb', where: ['Grocery', 'Deli', 'Costco'], unitCostRange: [5, 12], essential: false, buyAt: 'T-1d', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: '~1.5-2 oz/guest on a snack board.', claim: '~1.5–2 oz (~0.1 lb) of cured meat per guest is appropriate on a grazing snack board', sufficientWhen: 'A catering or charcuterie guide confirms ~1.5–2 oz/person as the standard snack-board meat portion' } },
    { id: 'p_crackers', item: 'Crackers + sliced baguette / crostini', category: 'food', qtyPerGuest: 0.2, unit: 'lb', where: ['Grocery', 'Bakery'], unitCostRange: [2, 5], essential: true, buyAt: 'T-3d', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'Vehicle for cheese/dips; crackers keep, so buy early.', claim: '~0.2 lb of crackers or baguette per guest provides the vehicle for cheese and dips on a grazing board', sufficientWhen: 'A hosting or catering guide confirms ~0.2 lb crackers/bread per guest as the standard grazing-board starch portion' } },
    { id: 'p_fruit', item: 'Fruit (grapes, berries, dried fruit) + olives/pickles', category: 'food', qtyPerGuest: 0.2, unit: 'lb', where: ['Grocery', 'Farmers market'], unitCostRange: [2, 4], essential: true, buyAt: 'T-1d', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'Color + fresh contrast on the board; low effort.', claim: '~0.2 lb of fresh fruit and accompaniments per guest provides color and fresh contrast on the grazing board', sufficientWhen: 'A hosting or catering guide confirms ~0.2 lb fruit/guest as a standard fresh-component portion on a grazing board' } },
    { id: 'p_veg', item: 'Crudité veg with hummus & ranch', category: 'food', qtyPerGuest: 0.2, unit: 'lb', where: ['Grocery'], unitCostRange: [1, 3], essential: true, buyAt: 'T-1d', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'The easy, healthy, allergy-friendly portion of the spread.', claim: '~0.2 lb of crudité veg per guest provides the allergy-friendly and health-conscious portion of a grazing spread', sufficientWhen: 'A catering or hosting guide confirms ~0.2 lb crudité/guest as the standard veg portion for a grazing-board party' } },
    { id: 'p_passed', item: 'Nuts, chips & salsa (popcorn, a warm dip)', category: 'food', qtyPerGuest: 2, unit: 'bites', where: ['Grocery', 'Trader Joe\'s'], unitCostRange: [0.5, 1.5], essential: true, buyAt: 'T-3d', note: 'Targets the ~6-8 bites/guest/hour rule across the whole spread; scatter bowls along the tour path.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'US hosting rule of thumb: ~6-8 app bites/guest/hour for an apps-only party; the board + these bowls together hit that.', claim: 'The grazing board plus scattered snack bowls together deliver ~6–8 app bites per guest per hour, the US apps-only party standard', sufficientWhen: '≥2 US catering or hosting guides confirm ~6–8 app bites/guest/hour as the standard for an apps-only party format' }, alternatives: ['Chips + salsa from store — cheapest warm snack option', 'Microwave popcorn (bulk) — if budget is tight'] },
    { id: 'p_sweet', item: 'Something sweet (cookies, brownies, a small cake)', category: 'food', qtyPerGuest: 1, unit: 'piece', where: ['Bakery', 'Grocery'], unitCostRange: [1, 3], essential: false, buyAt: 'T-1d', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'A small sweet closes the spread; optional for a casual drop-in.', claim: '~1 small sweet item per guest is optional but closes a housewarming grazing spread on a satisfying note', sufficientWhen: 'Host-coaching consensus — a dessert bite is a well-established hospitality convention for closing a grazing spread' }, alternatives: ['Grocery bakery cookies — cheaper than specialty', 'Brownies from box mix — budget DIY, tastes homemade'] },
    { id: 'p_beer', item: 'Beer (mix of light + craft)', category: 'beverage', qtyPerGuest: 1.5, unit: 'cans/bottles', where: ['Grocery', 'Liquor store', 'Total Wine'], unitCostRange: [1.5, 3], essential: true, buyAt: 'T-3d', note: '~1 drink/guest/hour split across beer/wine/soda; size to peak overlap.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'US rule of thumb ~1 alcoholic drink/guest/hour; allocate ~50% to beer for a casual crowd.', claim: '~1 alcoholic drink per guest per hour is the standard provision, with ~50% allocated to beer for a casual housewarming crowd', sufficientWhen: '≥2 US hosting or catering guides confirm ~1 drink/guest/hour and a ~50% beer allocation for a casual mixed-drink party' } },
    { id: 'p_wine', item: 'Wine (red + white + a sparkling)', category: 'beverage', qtyPerGuest: 0.4, unit: 'bottle', where: ['Wine shop', 'Total Wine', 'Grocery'], unitCostRange: [10, 20], essential: true, buyAt: 'T-3d', note: 'Rule of thumb: ~1 bottle per 2 guests over a 2-3h window; round up.', provenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'researched', sources: ['bar-provision-2026'], note: 'Grounded to bar-provision-2026: ~½ bottle of wine per drinker (~5 servings/750ml) is the source-stated rate.', claim: '~0.4 bottles of wine per guest (~½ bottle per drinker) is right for a 2–3h casual housewarming with beer and soda also available', sufficientWhen: '≥2 hosting or wine-service guides confirm ~5 glasses/bottle and ~½ bottle/wine-drinker for a casual short-format party' } },
    { id: 'p_cocktail', item: 'Signature cocktail spirits + mixers + garnish (one batch)', category: 'beverage', qtyFlat: 1, qtyPer: 12, unit: 'batch per 12 guests', where: ['Liquor store'], unitCostRange: [30, 55], essential: false, buyAt: 'T-3d', note: 'Batch in a pitcher so it pours itself — no bartender needed.', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'Optional; tied to the drinks decision.', claim: 'One batched signature cocktail per ~12 guests is sufficient to offer a festive zero-bartender option at a housewarming', sufficientWhen: 'Host-coaching convention — batch-per-event sizing is well-established hosting practice; no citation required beyond the drinks decision' } },
    { id: 'p_soda', item: 'Soda, sparkling water + juice (non-alcoholic)', category: 'beverage', qtyPerGuest: 2, unit: 'drinks', where: ['Grocery', 'Costco'], unitCostRange: [0.5, 1.5], essential: true, buyAt: 'T-3d', note: 'Always have great zero-proof options — drivers, kids, non-drinkers.', provenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'researched', sources: ['bar-provision-2026'], note: 'Grounded to bar-provision-2026: ~2–3 non-alcoholic servings/guest when alcohol is also offered is the source-stated rate.', claim: '~2–3 non-alcoholic drinks per guest is the right provision when alcohol is also available, to cover drivers, non-drinkers, and kids', sufficientWhen: '≥2 US hosting or catering guides confirm ~2–3 non-alcoholic drinks/guest as the standard when alcohol is also served' } },
    { id: 'p_water', item: 'Still water / cups for the sink station', category: 'beverage', qtyFlat: 1, unit: 'service', where: ['Home', 'Grocery'], unitCostRange: [0, 4], essential: true, buyAt: 'T-1d', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'A water pitcher + cups by the sink keeps people hydrated and out of your cabinets.', claim: 'A water pitcher and cups by the sink hydrate guests without requiring them to search a new home\'s cabinets', sufficientWhen: 'Host-coaching practice — standard new-home hospitality; no citation required' } },
    { id: 'p_ice', item: 'Ice', category: 'beverage', qtyPerGuest: 1.25, unit: 'lb', where: ['Grocery', 'Gas station'], unitCostRange: [0.2, 0.4], essential: true, buyAt: 'T0', note: 'COMMONLY FORGOTTEN. ~1-1.5 lb/guest for chilling drinks + cups; buy day-of.', provenance: { tier: 'trade-heuristic', confidence: 'high', verificationStatus: 'synthesized', note: 'US hosting rule of thumb: ~1-1.5 lb ice/guest, higher in warm weather or if chilling bottles in coolers.', claim: '~1–1.5 lb of ice per guest is sufficient for chilling drinks at a housewarming; scale up in warm weather or when chilling bottles in coolers', sufficientWhen: '≥2 US hosting or catering guides confirm ~1–1.5 lb ice/guest as the standard for a casual indoor-outdoor party' } },
    { id: 'p_cups', item: 'Cups, plates, cocktail napkins, small forks/picks', category: 'logistics', qtyPerGuest: 3, unit: 'sets', where: ['Grocery', 'Costco', 'Party store'], unitCostRange: [0.3, 0.8], essential: true, buyAt: 'T-3d', note: 'Plan ~2-3 cups/guest — people set drinks down and lose them. Compostable if you can.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'synthesized', note: 'Self-serve crowds churn through ~2-3 cups each; a marker for naming cups cuts waste.', claim: '~2–3 cups per guest is required for a self-serve housewarming where guests set drinks down and pick up fresh ones', sufficientWhen: '≥2 hosting guides confirm ~2–3 cups/guest as the standard for a self-serve drop-in party format' } },
    { id: 'p_flowers', item: 'Fresh flowers (entry + board area)', category: 'decor', qtyFlat: 2, unit: 'small arrangements', where: ['Florist', 'Trader Joe\'s', 'Farmers market'], unitCostRange: [8, 20], essential: false, buyAt: 'T-1d', note: 'The home is the decor — flowers + clean surfaces beat any theme.', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'Light, home-forward decor; deliberately not a heavy theme.', claim: '2 small flower arrangements — at the entry and near the food board — provide home-forward decor without imposing a heavy theme', sufficientWhen: 'Host-coaching convention — light, home-centered decor at a housewarming is established hosting practice' } },
    { id: 'p_candles', item: 'Unscented candles + one welcoming entry scent (optional)', category: 'decor', qtyFlat: 6, unit: 'candles', where: ['Grocery', 'Ikea', 'Amazon'], unitCostRange: [0.5, 3], essential: false, buyAt: 'T-3d', note: 'Unscented near food; a single scent at the entry only.', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'Ambiance, not theme.', claim: 'Unscented candles add ambiance to a housewarming without competing with food aromas or imposing a heavy theme', sufficientWhen: 'Host-coaching convention — unscented candles near food is standard hospitality practice' } },
    { id: 'p_entrykit', item: 'Entry kit: shoe tray + basket, coat hooks/rack, a small "welcome / shoes off" sign', category: 'logistics', qtyFlat: 1, unit: 'kit', where: ['Have/borrow', 'Amazon', 'Ikea'], unitCostRange: [0, 25], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: where 25 pairs of shoes and coats go in a new home.', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'New-home entry logistics are the housewarming-specific failure mode.', claim: 'A shoe tray, basket, coat space, and welcome sign are required to manage 25+ pairs of shoes and coats in a new home', sufficientWhen: 'Host-coaching practice — new-home entry logistics are a recognized housewarming-specific gap; no citation required' } },
    { id: 'p_cleanup', item: 'Trash + recycling bags, dish soap, sponges, paper towels, stain spray', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Costco'], unitCostRange: [8, 16], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: extra recycling bags for bottles/cans + stain spray for new-floor spills.', provenance: { tier: 'host-coaching', confidence: 'low', verificationStatus: 'synthesized', note: 'New floors/carpet raise the stakes on spill response.', claim: 'A cleanup kit with stain spray is especially important at a housewarming where spills on new floors or carpet have higher consequences', sufficientWhen: 'Host-coaching practice — immediate spill response on new floors is a recognized housewarming-specific concern' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['costco-cleaning-2026', 'trashbags-retail-2026'], lastVerified: '2026-08-16', claim: 'Cleanup supplies at warehouse pricing: 13-gallon trash bags about 10 cents each (200 for $20.42), paper towels about $1.97 a roll, dish soap with two refills $14.74, 24 sponges $12.47; the same bags run 11-15 cents each at grocery and big-box.', sufficientWhen: 'Current shelf prices for trash bags and paper towels at one warehouse club and one grocery store confirm the per-unit figures.' } },
  ],
