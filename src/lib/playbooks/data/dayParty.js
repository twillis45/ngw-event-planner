...
const dayParty = {
  ...
  decisions: [
    { id: 'music', label: 'DJ or a serious curated playlist?', options: ['Hire a DJ', 'Curated playlist + good speakers', 'DJ for peak hours + playlist on either side'], default: 'Hire a DJ', when: 'T-28d', blocks: ['music_setup', 'vendors'], weight: 'high', reversibility: 'costly', emotionalWeight: 'high', difmCapable: 'needs-host', deliversHeartMoment: true, priorityBasis: { rationale: 'The music is the hero of a day party — the DJ dropping the right song is the moment the floor fills, and getting it right is a taste call the host must own.', tier: 'reasoned' }, why: 'The music IS the party. A DJ reads the room and keeps the floor moving (old-school R&B → hip-hop → Afrobeats → line dances); in the DMV, a Go-Go set or band moves the rooftop better than anything. A deep playlist + real speakers is the budget version — never a phone on a tabletop speaker.' },
    { id: 'dresscode', label: 'Dress code / aesthetic', options: ['Grown-and-sexy', 'All-white', 'Summer linen / resort', 'Color theme (one bold color)', 'Come as you are'], default: 'Grown-and-sexy', defaultWhy: 'Grown-and-sexy is the default because this is the grown-folks afternoon, and it sets the photos and the energy without telling anyone what to go buy. Change it if you want the room to read as one picture — all-white or a single bold color do that, and both mean some guests have to shop for it — or to come as you are if you would rather nobody has to think about it.', when: 'T-21d', blocks: ['decor'], weight: 'low', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'The dress code sets the photos and the palette but costs nothing and lives on the invite — a cosmetic call the app can default.', tier: 'reasoned' }, why: 'A day party has a LOOK. The dress code sets the photos, the decor palette and the energy before anyone arrives — put it on the invite.' },
    { id: 'food', label: 'Food level (people came to vibe, not to eat heavy)', options: ['Light passed bites + grazing table', 'Food truck', 'Light bites only', 'Caterer drop-off small plates'], default: 'Light passed bites + grazing table', defaultWhy: 'Passed bites and a grazing table is the default because people came to move, not to sit down to a heavy meal, and hand-held food all afternoon keeps the floor busy. Change it past about 35 guests, where a food truck feeds the room without a line, or down to light bites only if the bar is where the budget needs to go.', when: 'T-21d', blocks: ['food', 'vendors'], costFactors: { 'Food truck': 1.4, 'Light bites only': 0.7, 'Caterer drop-off small plates': 1.3 }, costFactorProvenance: { tier: 'synthesized', confidence: 'medium', verificationStatus: 'synthesized', note: 'Cost factor heuristics need verification against actual pricing.', claim: 'A food truck costs ~1.4× the passed-bites baseline; light bites only cost ~0.7×; caterer drop-off costs ~1.3×', sufficientWhen: 'Verified against actual food truck and caterer quotes for day party headcounts in the 20-50 guest range' }, affects: ['p_bites'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'Food is deliberately light because people came to vibe, not eat — a modest, swappable call the app can default to passed bites.', tier: 'reasoned' }, why: 'This is NOT a sit-down dinner. Light, hand-held, all-afternoon food keeps people on their feet and dancing. A food truck is the easy crowd-pleaser at 35+ guests.' },
    { id: 'bar', label: 'Bar setup', options: ['Signature cocktail + batched punch + beer/wine + water', 'Self-serve punch + beer/wine', 'Hired bartender', 'BYOB + host provides mixers/ice'], default: 'Signature cocktail + batched punch + beer/wine + water', defaultWhy: 'A signature cocktail with a batched punch is the default because batching is what lets one person serve a crowd without a line, and the bar is the single biggest cost and ice lever here. Change it if you would rather hand the bar to a hired bartender for the afternoon, or if BYOB is how your crowd does it — you still supply the mixers and the ice either way.', when: 'T-21d', blocks: ['beverage_purchases', 'vendors'], weight: 'med', reversibility: 'reversible', emotionalWeight: 'low', difmCapable: 'can-derive', priorityBasis: { rationale: 'The bar is the single biggest cost-and-ice lever, but a signature-cocktail-plus-punch default serves a crowd and adjusts at the store.', tier: 'reasoned' }, why: 'A real bar separates a day party from a hangout. One signature cocktail + a big batched punch lets you serve a crowd without a line; beer/wine and plenty of WATER cover the rest. Drives the single biggest cost + ice lever.' },
    { id: 'venue', label: 'Where (daytime outdoor)', options: ['Backyard', 'Rooftop', 'Patio / deck', 'Rented outdoor space'], default: 'Backyard', when: 'T-28d', blocks: ['decor', 'logistics'], weight: 'high', reversibility: 'locked', emotionalWeight: 'med', difmCapable: 'needs-host', priorityBasis: { rationale: 'The outdoor spot fixes shade, DJ power, bathrooms, and the noise-permit question — the hardest call to reverse, and the host\'s to make.', tier: 'reasoned' }, why: 'Daytime outdoor is the whole point. Drives shade, seating, power for the DJ, bathroom access, and whether you need a noise permit.' },
  ],
  ...
  purchases: [
    { id: 'p_liquor', item: 'Spirits for signature cocktail + punch (rum / vodka / tequila / bourbon)', category: 'beverage', qtyPerGuest: 0.06, unit: 'bottle (750ml)', where: ['Liquor store', 'Costco', 'Total Wine'], unitCostRange: [15, 30], essential: true, buyAt: 'T-3d', note: 'Roughly one 750ml bottle yields ~16 cocktails; ~0.06 bottle/guest covers a signature drink + punch over a 5h afternoon.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'established-consensus', note: '~1 bottle of spirits per ~16 mixed drinks; 5h day-party pacing ~1 drink/guest/hr split across bar + punch.', claim: 'A 750ml spirits bottle yields ~16 cocktails; at 1 drink/guest/hour a 5h day party requires ~0.06 bottles per guest', sufficientWhen: 'Verified against US standard drink sizing (750 ml / 1.5 oz per drink = ~16 drinks) and typical outdoor cocktail-event consumption pacing' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['spirits-retail-2026', 'spirits-budgetbar-2026', 'liquor-shelf-tiers-2026'], lastVerified: '2026-08-16', claim: 'Spirits by shelf, per 750ml: BOTTOM SHELF $10-30 — named 2026 shelf prices put SKYY vodka at $12.95, Bacardi rum $12.95, Seagrams gin $12.95, Finlandia $15.95, Captain Morgan $15.95, Beefeater and Bombay $19.95, Jose Cuervo $20.95, El Jimador and Lunazul $21.95, Titos $25. MID SHELF is where most call brands land, roughly $25-50, and TOP SHELF is $50-200 or more. Most vodka buyers pay $18-40; tequila blanco $18-22; popular gin runs higher (Bombay Sapphire $30.99, St George Terroir $36.60).', sufficientWhen: 'Current shelf prices for one bottom-shelf and one top-shelf bottle of the same spirit at one retailer confirm the tier spread.' } },
    { id: 'p_punchbase', item: 'Punch base (juices, sparkling, lemonade, fruit)', category: 'beverage', qtyPerGuest: 0.6, unit: 'L', where: ['Grocery', 'Costco'], unitCostRange: [1.5, 3], essential: true, buyAt: 'T-3d', note: 'The batched punch is the workhorse — serve a crowd with no bar line. Dilute the batch ~10% with water so it is not syrupy.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' } },
    { id: 'p_mixers', item: 'Mixers (club soda, tonic, ginger beer, citrus)', category: 'beverage', qtyPerGuest: 1, unit: 'serving', where: ['Grocery', 'Costco'], unitCostRange: [0.25, 0.75], essential: true, buyAt: 'T-3d' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['mixers-retail-2026', 'mixers-walmart-2026'], lastVerified: '2026-08-18', claim: 'Basic mixers (tonic, club soda, ginger ale) in large-format 1L/2L bottles run ~$0.25-0.36 per 4oz serving; smaller-format mini cans run up to ~$1.27/serving. Band lowered 2026-08-18 to $0.25-0.75 to match the format a host actually buys (was $0.50-1.50).', sufficientWhen: 'A citrus/garnish price added to the mixer-per-serving figure closes the last gap in this band.' } },
    { id: 'p_beerwine', item: 'Beer + wine', category: 'beverage', qtyPerGuest: 2, unit: 'drinks', where: ['Liquor store', 'Costco', 'Grocery'], unitCostRange: [1.5, 4], essential: true, buyAt: 'T-3d', note: 'For guests who would rather sip — a standard mix runs roughly 40% wine, 35% beer, 25% spirits.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'established-consensus', note: '~5 drinks/guest over a 4-5h event (2 first hour, 1/hr after); 40/35/25 wine/beer/spirits split is a common catering rule.', claim: 'Guests consume ~5 drinks over a 4-5h event at a 40/35/25 wine/beer/spirits split; beer and wine together account for ~75% of volume', sufficientWhen: 'Verified against standard catering beverage consumption ratios for a 4-5h outdoor cocktail or social event' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['beer-retail-2026', 'beer-budget-2026', 'wine-retail-2026', 'wine-statewide-2026'], lastVerified: '2026-08-16', claim: 'This band is a SUM of separately-priced drink families, not a single quoted item: domestic lager $0.80-1.20 per 12oz (about $20-22 a 24-pack), craft $1.50-3.00; everyday table wine $8-15 a bottle, mid-range $15-30. Each component is cited to its own registered source; the summed band is therefore low-confidence by construction.', sufficientWhen: 'Current shelf prices for one pack of each named component at the same store, summed to the per-serving band, confirm the range.' } },
    { id: 'p_water', item: 'Water (bottled + dispenser) + non-alcoholic option', category: 'beverage', qtyPerGuest: 3, unit: 'servings', where: ['Grocery', 'Costco'], unitCostRange: [0.15, 1], essential: true, buyAt: 'T-3d', note: 'COMMONLY UNDER-BOUGHT: a daytime outdoor crowd dehydrates fast. Plenty of water + a real mocktail keeps everyone upright and is the backbone of not over-serving.' , costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['soda-12pack-2026', 'bottledwater-case-2026'], lastVerified: '2026-08-15', claim: 'A canned soda or bottled water runs $0.15-1.00 a serving: soda is $0.25-0.60 a can (average $0.38, store brand lower, craft $0.50-1.17), and basic bottled water is $0.13-0.25 a bottle in a 24-to-48 pack.', sufficientWhen: 'Re-checked against 12-pack and case pricing. The band is CANS AND BOTTLES ONLY - a mocktail, zero-proof spirit or punch base costs several times this per drink and is priced separately. Regional spread is real: Northeast and South run high, Midwest and West Coast low on discount-chain competition, and promotions cut 10-25%.' } },
    { id: 'p_ice', item: 'Ice (bar, coolers, punch)', category: 'beverage', qtyPerGuest: 2, unit: 'lb', where: ['Grocery', 'Gas station', 'Ice supplier'], unitCostRange: [0.2, 0.4], essential: true, buyAt: 'T0', note: 'COMMONLY FORGOTTEN. ~1.5-2 lb/guest, on the high end because it is outdoors in the heat and you are chilling drinks AND a punch.', provenance: { tier: 'trade-heuristic', confidence: 'medium', verificationStatus: 'established-consensus', note: '~1.5-2 lb ice/guest for an outdoor warm-weather event; double standard estimates outdoors.', claim: 'An outdoor day party in warm weather requires ~1.5-2 lb of ice per guest for drink chilling and the punch bowl', sufficientWhen: 'Verified against event-planning ice consumption guides for outdoor warm-weather gatherings of 4-5h duration' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['ice-retail-2026', 'ice-warehouse-2026'], lastVerified: '2026-08-16', claim: 'Bagged ice 2026: warehouse clubs run 10-12c per pound (a 20lb bag is $1.75-2.50 at Sams Club, $1.80-2.50 at Costco); grocery and gas-station bags cluster 23-31c/lb (BJs and 7-Eleven 20lb about $4.49-4.79, Giant 20lb $4.99, Publix 16lb $4.99); small bags and hardware stores reach 41-45c/lb. Convenience ice is more than four times warehouse ice per pound.', sufficientWhen: 'Current shelf prices for one 20lb bag at a warehouse club and one at a grocery store confirm the per-pound spread.' } },
    { id: 'p_bites', item: 'Charcuterie, wings & sliders (skewers, fruit)', category: 'food', qtyPerGuest: 5, unit: 'pieces', where: ['Grocery', 'Costco', 'Caterer'], unitCostRange: [1, 3], essential: true, buyAt: 'T-1d', note: 'Hand-held, all-afternoon food. ~5-6 bites/guest for a no-meal day party; a grazing table holds the room without table service. In the DMV, mambo/mumbo-sauce wings are the recognized bite.', alternatives: ['Costco party platters — cheaper per bite, minimal prep', 'Frozen appetizers (Trader Joe\'s) — budget option, bake day-of', 'Fruit + veggie tray + dip — cheapest grazing option if budget is tight'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['charcuterie-grocery-2026', 'wings-extension-2026', 'costco-groundbeef-2026', 'buns-walmart-2026'], lastVerified: '2026-08-18', claim: 'Passed pieces from a mixed board, summed from components. CHARCUTERIE: specialty cheese about $15/lb and cured meats about $20/lb, with a DIY medium board $50-120 for 5-15 people. WINGS $2.49-2.67/lb retail. SLIDERS take ground beef at $3.29/lb bulk plus buns at about $0.50 each. Divided across the pieces each guest takes, that lands in this band; a cured-meat-heavy board pushes it to the ceiling.', sufficientWhen: 'Board ingredient cost plus a wing and slider count, divided by pieces served, confirms the per-piece figure.' }, },
    { id: 'p_garnish', item: 'Fresh garnish + punch fruit (citrus, mint, berries)', category: 'food', qtyFlat: 1, unit: 'kit', where: ['Grocery', 'Farmers market'], unitCostRange: [15, 30], essential: true, buyAt: 'T-1d', alternatives: ['Frozen fruit — cheaper for the punch bowl, works fine once thawed'] , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['garnish-produce-2026', 'bls-saladveg-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced garnish components. Limes $0.39 each at Target, with a USDA weekly retail report showing $0.69-0.79/lb and $0.12-0.69 individually. Fresh mint sells as a 0.5oz clamshell at $1.99-2.80, NOT as a loose bunch. Berries: blueberries $3.99 a pint, strawberries $2.99/lb. A party-sized garnish tray of citrus, mint and berries sums into this band; the mint clamshell format is what keeps it from being cheaper.', sufficientWhen: 'Shelf prices for a bag of limes, two mint clamshells and a pint of berries, summed, confirm the band.' }, },
    { id: 'p_cups', item: 'Cups, plates, napkins, cocktail picks, cutlery (disposable)', category: 'rental', qtyPerGuest: 3, unit: 'set', where: ['Party store', 'Costco', 'Grocery'], unitCostRange: [0.2, 0.7], essential: true, buyAt: 'T-3d', note: 'Day parties go through cups — people set one down and grab a fresh one. Buy 3x.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['disposables-bulk-2026', 'disposables-partyqty-2026'], lastVerified: '2026-08-16', claim: 'Disposable tableware runs about 6 cents a plate in bulk packs (roughly $18 per 300) and $0.25-0.40 a plate at a grocery store, with bulk restaurant supply at $0.08-0.15; cups, cutlery and napkins bought in bulk alongside them save $100-200 across a party.', sufficientWhen: 'Current shelf prices for one bulk plate pack and the same item at a grocery store confirm the per-piece spread.' } },
    { id: 'p_decor', item: 'Decor + photo-wall backdrop (balloons, greenery, signage, theme palette)', category: 'decor', qtyFlat: 1, unit: 'kit', where: ['Party store', 'Amazon', 'Florist'], unitCostRange: [60, 200], essential: false, buyAt: 'T-3d', note: 'The vibe/decor moment + a photo wall in the dress-code palette is what turns a backyard into a day party — and gives the room its photos.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['paperlust-centerpieces-2026', 'balloon-kits-2026', 'balloon-bulk-2026', 'banner-retail-2026'], lastVerified: '2026-08-18', claim: 'A SUM of separately-priced components. FLORALS at the DIY tier are $20-50 a table (elevated DIY $50-100; a florist starts at $75-150 and sits above this band). BALLOONS: a garland kit $12.09 wholesale for 94-120, or $20.99-36.99 for a 150-piece specialist kit. BANNER $1.25 party store, or $15-30 for a printed 18x24in foam board. Summed, that brackets this band. Florist delivery and setup ($200-500) is excluded — these rows are host-assembled.', sufficientWhen: 'A DIY floral cost for the table count, plus one balloon kit and a banner, summed, confirm the band.' }, },
    { id: 'p_speakers', item: 'Speaker / PA rental (if no DJ or DJ does not bring sound)', category: 'logistics', qtyFlat: 1, unit: 'system', where: ['AV rental', 'Music store', 'Amazon'], unitCostRange: [80, 250], essential: false, buyAt: 'T-3d', note: 'If you went the playlist route, real speakers are non-negotiable — never a phone on a Bluetooth puck. Confirm power/outlets outside.' , provenance: { tier: 'estimate', confidence: 'low', verificationStatus: 'synthesized' }, costProvenance: { tier: 'researched', confidence: 'medium', verificationStatus: 'cited', sources: ['pa-rental-local-2026', 'pa-rental-2026'], lastVerified: '2026-08-18', claim: 'PA rental for one day. A local self-collected speaker starts at about $60 per 24 hours and a pair of powered JBL PRX715s is about $220 a day - which is this band. FULL-SERVICE EVENT SOUND IS A DIFFERENT PURCHASE and is excluded: a basic two-speaker system with a mixer is $200-500 for a half day delivered, larger configurations $800-2,000 a day with crew, and full event sound $500-5,000+. The band prices the host collecting speakers, not a company arriving to run them.', sufficientWhen: 'One local per-day speaker rate against one delivered-and-crewed quote confirms which purchase this band is pricing.' }, },
    { id: 'p_cleanup', item: 'Trash + recycling bags, paper towels, foil/containers for leftovers', category: 'cleanup', qtyFlat: 1, unit: 'kit', where: ['Grocery'], unitCostRange: [7, 18], essential: true, buyAt: 'T-3d', note: 'COMMONLY FORGOTTEN: a recycling bag for all the cans/bottles + containers for leftover food and punch.' , costProvenance: { tier: 'researched', confidence: 'low', verificationStatus: 'cited', sources: ['costco-cleaning-2026', 'trashbags-retail-2026'], lastVerified: '2026-08-15', claim: 'A cleanup kit runs $7-18 as the SUM of its parts: about a dozen trash and recycling bags at 10 cents each from a warehouse or 11-15 cents at grocery, two rolls of paper towels at about $1.97 warehouse, and a canister of wipes at about $4.27 or a dish-soap pack at $14.74 shared across events.', sufficientWhen: 'CONFIDENCE IS LOW ON PURPOSE: no source prices a cleanup kit, because nobody sells one. This band is a sum of individually-priced components, so treat it as an envelope rather than a quote. The spread is the CHANNEL - warehouse packs against a grocery shelf - and a host who already owns soap and towels lands well under the floor. Kits that also carry gloves, foil or to-go containers are a different bundle.' } },
  ],
  ...
  risks: [
    ...
    { id: 'r_noise', trigger: 'Noise complaint / neighbors / no permit — police shut the music down', severity: 'high', mitigation: 'Check the daytime noise ordinance + permit rules at 3 weeks out; give neighbors a heads-up (or invite them); point speakers away from homes; keep it to the posted daytime hours.' },
    ...
    { id: 'r_weather', trigger: 'Rain or extreme heat with no shade plan', severity: 'high', mitigation: 'Check the forecast at 5 days out; secure canopies + a covered/indoor fallback; provide shade + water; shift earlier to dodge peak heat.' },
