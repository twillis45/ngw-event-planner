...
function buildBranch(slug, s) {
  ...
  const decisions = [
    { id: 'mil_color_guard', label: (slug === 'navy' || slug === 'coastguard') ? 'Who renders honors?' : `Who posts ${s.colors}?`, options: s.honorOpts, default: s.honorOpts[s.honorDefault || 0], when: 'T-45d', blocks: ['run_of_show'], weight: 'high', why: `A ${(slug === 'navy' || slug === 'coastguard') ? 'honor' : 'color'} guard posts and retires ${s.colors} — book early; they run on their own calendar.`, militaryContext: mc('color_guard', [s.protocolSrc]) },
    { id: 'mil_shadowbox', label: 'The shadowbox', options: ['Commission a builder', `Have the ${s.unit} build it`, 'Build it ourselves', 'Skip it'], default: 'Commission a builder', when: 'T-45d', weight: 'high', why: 'The shadowbox holds the folded flag, insignia, and medals — the career’s keepsake and the longest-lead item. Commission it early.', militaryContext: mc('shadowbox', [s.rsoSrc]) },
...
function buildPromotion(event) {
  ...
  const decisions = [
    { id: 'prom_pinning', label: 'Who pins the new rank?', options: ['Spouse + a mentor', 'Family members', 'The presiding officer', 'A combination'], default: 'Spouse + a mentor', when: 'T-14d', weight: 'high', emotionalWeight: 'high', deliversHeartMoment: true, why: 'The pinning is the emotional core of a promotion — decide who does the honors.', militaryContext: mc('pinning', [s.protocolSrc]) },
