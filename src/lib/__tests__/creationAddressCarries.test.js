...
// ─── THE VERDICT RATCHET MOVED ───────────────────────────────────────────────
// A ratchet lived here for "no surface re-derives 'is the venue set?'" — the
// class PART A of venueSourceProof cannot see, because these surfaces all read
// THROUGH venueFor and then recomputed the rule as `!!vf.name` / `!vf.name`.
// It was narrow by design: four hand-listed files.
//
// It is now PART D of src/__tests__/venueSourceProof.test.js, widened to scan
// ALL of src/lib and hostv2/src with a baseline escape hatch, so the venue rules
// live in ONE file and a new surface cannot join the class unnoticed. All four
// files it named are inside PART D's walk, and its canary assertions are
// reproduced there verbatim. This file keeps its own subject: the SEED — the
// street the host typed at creation reaching the venue, and the surface guard
// above that proves the blocker reads it.
