...
export const MONEY_PROVENANCE = {
  ...
  'sourcing.tiers': {
    ...
    sources: [],
    count: 3,
    appliesTo: 'sourcing.js#SOURCING_TIERS (butcher 1.0 / costco 0.85 / grocery 1.18), applied by playbooks/index.js#srcFactorFor ONLY to protein lines that resolve no per-tier range',
    claim: 'Bulk sourcing re-prices a protein line at 0.85x the butcher baseline; a one-stop grocery run at 1.18x — wherever no authored or canonical per-tier price exists for that line.',
    note: 'MEASURED, not asserted. The file USED TO STATE that the grocery premium was "honestly backed by the raw per-channel data below" — CANONICAL_PROTEIN_PRICES, 10 proteins with real per-channel ranges and URLs. Computing the channel ratios from that table across all 10 proteins (midpoint to midpoint) gives a mean costco ratio of 0.706 and a mean grocery ratio of 1.064. The shipped factors are 0.85 and 1.18. So the table corroborates the DIRECTION of both factors and neither MAGNITUDE: the shipped costco factor is ~20% less generous than its own data and the shipped grocery premium ~11% steeper. The claim of backing was made in good faith and is directionally true; it does not survive arithmetic. '
        + 'RE-MEASURED 2026-09-19 — AND THE ANSWER IS THAT THE TABLE CANNOT DERIVE THESE FACTORS, which is why no number moved and the claim of backing was withdrawn from sourcing.js instead. Two findings, kept separate. '
        + '(1) NO SINGLE BASIS. Five defensible derivations of the same table disagree: mean of per-item ratios 0.706 / 1.064; ratio of the channel means 0.733 / 1.059; median of per-item ratios 0.690 / 1.048; ratio of the low bounds 0.740 / 1.083; ratio of the high bounds 0.730 / 1.046. Costco spans 0.69-0.74 and grocery 1.05-1.08. Every one of them is far from 0.85 / 1.18 and none of them is THE number, and the withdrawn sentence never named a basis. Re-pricing protein on a basis chosen after the fact is a product decision, not a research finding. '
        + '(2) WRONG POPULATION — the more serious of the two. playbooks/index.js#srcFactorFor gives a protein line factor 1.0 whenever it resolves a per-tier range (its own authored sourcingPrices, or CANONICAL_PROTEIN_PRICES), so these factors apply to exactly the lines that table does NOT price. Measured across ALL_PLAYBOOKS on a non-default tier: 6 protein lines authored, 32 covered by the canonical table, 0 falling through to the factor. The factor is the fallback for host-authored protein the table misses, and the table is the complement of its population. Deriving a fallback for uncovered items from the covered items would be an analogy dressed as arithmetic.',
    sufficientWhen: 'A dated channel-price comparison is registered that prices the population this factor actually reaches — protein the canonical table does not cover — and the factors are reconciled to it. Deriving them from CANONICAL_PROTEIN_PRICES would NOT be enough on its own: that table prices the complement of this factor\'s population, it supports five different answers depending on the basis chosen, and its per-line retail-guide URLs are not registered in MONEY_SOURCES, so the predicate would fail even if the arithmetic were agreed.',
    hostExplanation: true,
  },
