...
async def send_envelope(
    ...
    try:
        ...
        # Logged server-side only. DocuSign exception text can carry account and
        # token detail; it must not reach the caller.
        log.error("Envelope creation exception: %s", e)
        raise HTTPException(status_code=500, detail="DocuSign envelope creation failed")
